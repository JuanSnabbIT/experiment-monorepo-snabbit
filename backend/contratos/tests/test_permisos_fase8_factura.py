"""Fase 8: regresion de permisos tras migrar FacturaContratoViewSet de
...
class FacturaContratoFase8PermisosTest(TestCase):
    URL = "/api/facturas-contrato/"

    def setUp(self):
        self.client = APIClient()
        self.empresa = Empresa.objects.create(nombre="Empresa Factura Fase8", direccion_principal="Dir")
    ...
    def test_rol_finanzas_puede_listar(self):
        user, _ = crear_usuario_en_rol(self.sucursal, "finanzas", sufijo="fase8-factura-finanzas")
    ...
    def test_rol_sin_permiso_recibe_403(self):
        user, _ = crear_usuario_en_rol(self.sucursal, "tecnico", sufijo="fase8-factura-403")
        self.client.force_authenticate(user=user)
    ...
    def test_finalizar_rol_finanzas_pasa_el_permiso(self):
        user, _ = crear_usuario_en_rol(self.sucursal, "finanzas", sufijo="fase8-factura-finalizar-ok")
        self.client.force_authenticate(user=user)
    ...
    def test_finalizar_rol_sin_permiso_recibe_403(self):
        user, _ = crear_usuario_en_rol(self.sucursal, "tecnico", sufijo="fase8-factura-finalizar-403")
        self.client.force_authenticate(user=user)
