...
class ContratoEmpresaClienteViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        # "Contratos" de DetalleCliente.tsx. Misma naturaleza de solo-lectura
        # que list/retrieve.)
        if self.action in (
            "list",
            "retrieve",
    ...
    def _normalizar_cantidad_licencia(self, value, *, default=1):
        cantidad = default if value in (None, "") else value
        try:
            cantidad_normalizada = int(cantidad)
        except (TypeError, ValueError):
            raise serializers.ValidationError({"cantidad": ["Debe ser un numero entero mayor a 0."]})
    ...
    def _asegurar_licencias_para_contrato_tipo_licencia(self, contrato):
        if contrato.tipo != "licencia":
            return
        if ContratoLicencia.objects.filter(contrato=contrato).exists():
            return
        raise serializers.ValidationError(
            {
                "detail": [
                    "Debes agregar al menos una licencia para contratos de tipo licencia."
                ]
            }
        )
    ...
    def _reemplazar_licencias(self, contrato, licencias_data):
        ...
        for item in licencias_data or []:
            ...
            try:
                campos_licencia = self._build_campos_licencia_contrato(
                    contrato=contrato,
                    licencia=licencia,
                    item=item,
                    cantidad_default=1,
    ...
    def _guardar_borrador_unificado(self, contrato, payload, *, crear=False):
        ...
        try:
            self._asegurar_licencias_para_contrato_tipo_licencia(contrato)
        except serializers.ValidationError as exc:
            errores["licencias"] = exc.detail
    ...
    def crear_completo(self, request):
        ...
        with transaction.atomic():
            ...
            try:
                ...
                raise
            # Auto-congelar el documento v2.9 desde la plantilla
    ...
    def enviar_aprobacion(self, request, pk=None):
        ...
        # Documento unico v2.9: se congela aqui mismo si aun no existe o si
        # la plantilla cambio de version, igual que lo hace de forma
        # perezosa construir_pdf_contrato/_resolver_pdf_contrato.
        if contrato.plantilla:
            from contratos.adaptadores import AdaptadorContratoB2B
            from contratos.motor_v29 import generar_o_recongelar_documento_v29
            ...
            return Response(
                {
                    "detail": (
                        "El contrato no tiene una plantilla asignada ni secciones generadas. "
                        "Selecciona una plantilla antes de enviarlo a aprobación."
                    )
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
...
# =====================================================================
# API V2 — Motor de Plantillas V2 (unico motor soportado: documento v2.9)
# =====================================================================
