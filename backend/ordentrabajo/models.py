...
class OrdenDeTrabajo(ModeloBaseHistorico):
    empresa = models.ForeignKey(
        ...
        related_name="empresa_ot",
    )
    cliente = models.ForeignKey(
        "empresas.Empresa",
        on_delete=models.CASCADE,
        verbose_name="cliente_orden_trabajo",
        related_name="cliente_ot",
    )
    ...
    prioridad = models.CharField(
        max_length=20, choices=PRIORIDAD, default="1", verbose_name="Prioridad"
    )
    ...
    responsable_empresa = models.ForeignKey(
        ...
        related_name="responsable_ot",
    )
    solicitante_empresa = models.ForeignKey(
        "empresas.UsuarioEmpresa",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        verbose_name="Solicitante",
        related_name="solicitante_ot",
    )
...
class UsuarioAsignadoOT(ModeloBaseHistorico):
    ...
    usuario_empresa = models.ForeignKey(
        "empresas.UsuarioEmpresa", on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=(
                    (
                        models.Q(usuario_empresa__isnull=False)
                        & models.Q(usuario_externo__isnull=True)
                        & models.Q(correo_usuario_externo__isnull=True)
                    )
                    | (
                        models.Q(usuario_empresa__isnull=True)
                        & models.Q(usuario_externo__isnull=False)
                    )
                ),
    ...
    def clean(self):
        super().clean()
        if self.usuario_empresa and (
            self.usuario_externo or self.correo_usuario_externo
        ):
            raise ValidationError(
                "No puede asignar ambos: usuario_empresa y usuario_externo/correo_usuario_externo. Debe elegir solo uno."
            )
...
class DetalleTrabajo(ModeloBaseHistorico):
    ...
    insumo = models.OneToOneField(
        "bodegas.GuiaSalida", on_delete=models.CASCADE, blank=True, null=True
    )
...
class SeguimientoDetalleTrabajo(ModeloBaseHistorico):
    detalle_trabajo = models.ForeignKey(
        DetalleTrabajo,
        on_delete=models.CASCADE,
...
class CierreAdministrativoOT(ModeloBaseHistorico):
    orden = models.OneToOneField(
        OrdenDeTrabajo, on_delete=models.CASCADE, related_name="cierre_administrativo"
    )
    usuario = models.ForeignKey(
