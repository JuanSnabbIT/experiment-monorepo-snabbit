# Generated by Django 5.1.15 on 2026-02-23 15:28

import django.db.models.deletion
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('auth', '0012_alter_user_first_name_max_length'),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Empresa',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('fecha_creacion', models.DateTimeField(auto_now_add=True)),
                ('fecha_modificacion', models.DateTimeField(auto_now=True)),
                ('nombre', models.CharField(max_length=255)),
                ('sitio_web', models.TextField(blank=True, null=True)),
                ('direccion_principal', models.CharField(max_length=250)),
                ('logo', models.TextField(blank=True, null=True)),
                ('firma_empresa', models.TextField(blank=True, null=True)),
                ('recargo', models.PositiveIntegerField(default=0)),
                ('rut_empresa', models.CharField(blank=True, max_length=100, null=True)),
                ('telefono', models.CharField(blank=True, max_length=20, null=True)),
                ('email', models.EmailField(blank=True, max_length=254, null=True)),
                ('ppm', models.DecimalField(decimal_places=2, default=1, max_digits=5)),
                ('uuid', models.UUIDField(blank=True, editable=False, null=True, unique=True)),
            ],
            options={
                'verbose_name': 'Empresa',
                'verbose_name_plural': 'Empresas',
            },
        ),
        migrations.CreateModel(
            name='RelacionEmpresa',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('fecha_creacion', models.DateTimeField(auto_now_add=True)),
                ('fecha_modificacion', models.DateTimeField(auto_now=True)),
                ('tipo_relacion', models.CharField(default='prestador-cliente', max_length=50)),
                ('cliente', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='relaciones_como_cliente', to='empresas.empresa')),
                ('prestador_servicios', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='relaciones_como_prestador_servicios', to='empresas.empresa')),
            ],
            options={
                'verbose_name': 'Relación entre Empresas',
                'verbose_name_plural': 'Relaciones entre Empresas',
                'unique_together': {('prestador_servicios', 'cliente')},
            },
        ),
        migrations.AddField(
            model_name='empresa',
            name='clientes',
            field=models.ManyToManyField(through='empresas.RelacionEmpresa', to='empresas.empresa'),
        ),
        migrations.CreateModel(
            name='SucursalEmpresa',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('fecha_creacion', models.DateTimeField(auto_now_add=True)),
                ('fecha_modificacion', models.DateTimeField(auto_now=True)),
                ('nombre', models.CharField(max_length=250)),
                ('direccion', models.CharField(blank=True, max_length=255, null=True)),
                ('telefono', models.CharField(blank=True, max_length=20, null=True)),
                ('email', models.EmailField(blank=True, max_length=254, null=True)),
                ('region', models.IntegerField(default=0)),
                ('provincia', models.IntegerField(default=0)),
                ('comuna', models.IntegerField(default=0)),
                ('empresa', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='sucursales', to='empresas.empresa')),
            ],
            options={
                'verbose_name': 'Sucursal de la Empresa',
                'verbose_name_plural': 'Sucursales de la Empresa',
            },
        ),
        migrations.CreateModel(
            name='UsuarioEmpresa',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('fecha_creacion', models.DateTimeField(auto_now_add=True)),
                ('fecha_modificacion', models.DateTimeField(auto_now=True)),
                ('fecha_ingreso', models.DateField(blank=True, null=True)),
                ('fecha_contrato', models.DateField(blank=True, null=True)),
                ('cargo', models.CharField(blank=True, max_length=150, null=True)),
                ('estado', models.CharField(choices=[('1', 'Activo'), ('2', 'Desactivo')], default='1', max_length=1)),
                ('grupos', models.ManyToManyField(blank=True, to='auth.group')),
                ('sucursal', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='usuarios', to='empresas.sucursalempresa')),
                ('usuario', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'Usuario de la Empresa',
                'verbose_name_plural': 'Usuarios de la Empresa',
                'ordering': ['fecha_creacion'],
            },
        ),
    ]
