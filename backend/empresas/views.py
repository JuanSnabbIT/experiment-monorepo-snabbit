...
class EmpresaViewSet(viewsets.ModelViewSet):
    ...
    def usuarios_de_clientes(self, request, pk=None):
        ...
        try:
            ...
            return Response({'detail': 'Empresa no existe'}, status=status.HTTP_404_NOT_FOUND)
...
class UsuarioEmpresaViewSet(viewsets.ModelViewSet):
    ...
    def detalle_usuario(self, request):
        ...
        if not usuario_id:
            return Response({"error": "El parametro 'usuario_id' es requerido."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            usuario_empresa = UsuarioEmpresa.objects.get(usuario__id=usuario_id)
        except UsuarioEmpresa.DoesNotExist:
            return Response({"error": "No se encontró un UsuarioEmpresa con el id proporcionado."}, status=status.HTTP_404_NOT_FOUND)
    ...
    def ultimas_actividades(self, request, pk=None):
        try:
            ...
            return Response({"error": "No se encontró un UsuarioEmpresa para el usuario actual."}, status=status.HTTP_404_NOT_FOUND)
    ...
    def usuarios_por_empresas(self, request):
        ...
        if not isinstance(empresa_ids, list) or not all(isinstance(i, int) for i in empresa_ids):
            return Response(
    ...
    @action(detail=False, methods=["get"], url_path=r'empresa/(?P<empresa_id>\d+)/tecnicos', url_name="tecnicos-por-empresa")
