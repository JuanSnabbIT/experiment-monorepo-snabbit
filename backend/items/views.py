...
class ProveedorEmpresaViewSet(viewsets.ModelViewSet):
    ...
    def no_asociados_a_item(self, request):
        ...
        if not item_id:
            return Response({"error": "Debe proporcionar un 'item_id' como parámetro."}, status=400)
    ...
    def asociar_item(self, request, pk=None):
        ...
        if not item_id:
            return Response({"error": "Debe proporcionar un 'item_id' en el cuerpo de la solicitud."}, status=400)
        try:
        ...
        return Response({"message": f"Item '{item.nombre}' asociado al proveedor '{proveedor.nombre}'."}, status=200)
    ...
    def desasociar_item(self, request, pk=None):
        ...
        if item not in proveedor.items_proveedor.all():
            return Response({"error": "El item no está asociado a este proveedor."}, status=400)

        proveedor.items_proveedor.remove(item)
        return Response({"message": f"Item '{item.nombre}' desasociado del proveedor '{proveedor.nombre}'."}, status=200)
...
class ItemEmpresaViewset(viewsets.ModelViewSet):
    ...
    def items_sin_proveedor(self, request, *args, **kwargs):
        ...
        if not proveedor_id:
            return Response({"detail": "El campo 'proveedor_id' es requerido."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def asociar_proveedor(self, request, pk=None):
        ...
        if not proveedor_id:
            return Response({"error": "Debe proporcionar un 'proveedor_id' en el cuerpo de la solicitud."}, status=400)
        ...
        item.proveedores_empresa.add(proveedor)
        return Response({"message": f"Proveedor '{proveedor.nombre}' asociado al item '{item.nombre}'."}, status=200)
    ...
    def desasociar_proveedor(self, request, pk=None):
        ...
        if proveedor not in item.proveedores_empresa.all():
            return Response({"error": "El proveedor no está asociado a este item."}, status=400)

        item.proveedores_empresa.remove(proveedor)
        return Response({"message": f"Proveedor '{proveedor.nombre}' desasociado del item '{item.nombre}'."}, status=200)
    ...
    def movimientos_stock(self, request, pk=None, empresa_pk=None):
        ...
        movimientos_qs = (
            ...
            .order_by("-fecha_creacion")    # o el campo de fecha que uses
        )
        ...
        serializer = MovimientoStockSerializer(
            movimientos_qs, many=True, context={"request": request}
        )
        return Response(serializer.data)

    @action(detail=True, methods=['get'], url_path="proveedores")
    def obtener_proveedores(self, request, pk=None):
        return Response(ProveedorEmpresaSerializer(self.get_object().proveedores_empresa.all(), many=True).data, status=status.HTTP_200_OK)
...
class ProveedorEmpresaItemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if proveedor_empresa_pk:
            return ItemEmpresa.objects.filter(proveedores_empresa__id=proveedor_empresa_pk)
