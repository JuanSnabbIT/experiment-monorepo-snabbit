...
class ItemEmpresa(ModeloBase):
    nombre = models.CharField(max_length=250)
    descripcion_corta = models.CharField('Algo que lo referencie', max_length=45, blank=True, null=True)
    fabricante = models.ForeignKey(Fabricante, blank=True, null=True, on_delete=models.SET_NULL, help_text="Dejar en blanco si el fabricante es igual al proveedor")
    categoria = models.ForeignKey(Categoria, on_delete=models.SET_NULL, null=True, blank=True)
    empresa = models.ForeignKey("empresas.Empresa", on_delete=models.CASCADE)
    proveedores_empresa = models.ManyToManyField(ProveedorEmpresa, blank=True, related_name='items_proveedor')
    comentarios = models.TextField(blank=True, null=True)
    codigo_barras = models.TextField(blank=True, null=True, unique=True)
    campos_adicionales = models.ManyToManyField(CampoAdicionalProveedor, through=CampoAdicionalItem, blank=True)

    def __str__(self):
        return '%s de %s'%(self.nombre, self.empresa.nombre)

class ImagenItem(models.Model):
    item = models.ForeignKey(ItemEmpresa, related_name='imagenes', on_delete=models.CASCADE)
