...
from .models import ArchivoCompra, Bodega, Compra, ImagenDeItemEnTomaInventario, ItemEnCompra, ItemsGuiaSalida, GuiaSalida, MovimientoStock, TomaInventario, OrdenCompra, ItemEnOrdenCompra, StockItemEnBodega, ItemOrdenCompraEnStock, ItemEnTomaInventario, EstadoTomaInventario
from .serializers import AjusteStockSerializer, ArchivoCompraSerializer, BodegaSerializer, CompraCreateSerializer, CompraSerializer, EstadoTomaInventarioSerializer, ImagenDeItemEnTomaInventarioSerializer, ItemEnCompraSerializer, ItemEnTomaInventarioSerializer, ItemsGuiaSalidaSerializer, GuiaSalidaSerializer, MovimientoStockSerializer, OrdenCompraCreateSerializer, StockInicialSerializer, TomaInventarioSerializer, OrdenCompraSerializer, ItemEnOrdenCompraSerializer, StockItemEnBodegaSerializer, ItemOrdenCompraEnStockSerializer, TomaInventarioCrearSerializer, MultipleImagenesSerializer
...
from .functions import crear_equipos_para_items_guia, generar_orden_de_compra, generar_pdf_bodega, generar_pdf_bodega_resumido
from .estados_modelo import ESTADOS_OC
from .movimientos import registrar_entrada, registrar_salida, registrar_devolucion, registrar_ajuste_inventario
...
class BodegaViewSet(viewsets.ModelViewSet):
    ...
    def generar_pdf(self, request, pk=None):
        ...
        stock_items = StockItemEnBodega.objects.filter(bodega=bodega).select_related('item__fabricante', 'item__categoria')
        if not stock_items.exists():
            return Response({"detail": "La bodega no tiene items en stock."}, status=400)
        ...
        for stock in stock_items:
            ...
            datos_bodega["items"].append({
                ...
                "fabricante": item.fabricante.nombre if item.fabricante else "Sin fabricante",
        ...
        response['Content-Disposition'] = f'attachment; filename="bodega_{bodega.nombre}.pdf"'
        return response
    ...
    def generar_pdf_resumido(self, request, pk=None):
        ...
        stock_items = StockItemEnBodega.objects.filter(bodega=bodega).select_related('item__fabricante', 'item__categoria')
        if not stock_items.exists():
            return Response({"detail": "La bodega no tiene ítems en stock."}, status=400)
        ...
        # Devolver el PDF como respuesta
        response = HttpResponse(pdf_buffer, content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="reporte_categorias_bodega_{bodega.nombre}.pdf"'
        return response

class TomaInventarioViewSet(viewsets.ModelViewSet):
    queryset = TomaInventario.objects.all().prefetch_related('bodegas', 'creado_por__usuario')
    ...
    def crear_toma_inventario(self, request):
        ...
        with transaction.atomic():
            ...
            EstadoTomaInventario.objects.create(
                toma_inventario=toma, estado="pendiente", usuario=usuario, fecha_cambio=request.data.get("fecha_cambio")
            )
    ...
    def estados_en_toma(self, request, pk=None):
        ...
        qs = (
            toma.estados
                .select_related("usuario__usuario")   # optimiza la FK a UsuarioEmpresa → User
        ...
        with transaction.atomic():
            ...
            for item_toma in toma.itementomainventario_set.select_related("stock_item"):
                ...
                if item_toma.cantidad_encontrada != stock_item.cantidad:
                    ...
                    registrar_ajuste_inventario(stock_item=stock_item, usuario=usuario, origen=item_toma, cantidad=item_toma.cantidad_encontrada, descripcion="Items ajustados desde una toma de inventario")
...
class ItemEnTomaInventarioViewSet(viewsets.ModelViewSet):
    queryset = ItemEnTomaInventario.objects.select_related("stock_item__item", "stock_item__bodega", "toma_inventario")
    serializer_class = ItemEnTomaInventarioSerializer

    @action(detail=False, methods=["get"], url_path="buscar", url_name="buscar-por-toma-y-codigo")
...
class OrdenCompraViewSet(viewsets.ModelViewSet):
        ...
        for orden_compra in ultimas_ordenes:
            ...
            for item in items_oc:
                eventos.append({
                    ...
                    "usuario": orden_compra.creado_por.usuario.get_nombre() if orden_compra.creado_por and orden_compra.creado_por.usuario else "Desconocido"
                })
    ...
    def add_item(self, request, pk=None):
        try:
            ...
            if not item_id:
                return Response({"error": "Se requiere item, cantidad y precio"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"message": "Item agregado con éxito", "item_en_orden": {
                "id": item_en_orden.id,
    ...
    def ordenes_por_empresa(self, request, empresa_id=None):
        ...
        estados = request.query_params.getlist('estado', [])  # List para permitir múltiples valores
        oc_clientes = request.query_params.getlist('oc_cliente', [])  # List para múltiples clientes

        # Construir el queryset base
        queryset = self.get_queryset().filter(Q(oc_cliente_id=empresa_id) | Q(oc_empresa_id=empresa_id))
    ...
    def pasar_enviado_proveedor(self, request, pk=None):
        ...
        reenviar = request.data.get('reenviar', False)  # Se espera un booleano para indicar si es un reenvío
        ...
        try:
            ...
            if not reenviar:  # Cambiar el estado solo si no es un reenvío
                orden.estado = "3"
                orden.save()
            return Response({'message': f'Correo enviado exitosamente a {email_destinatario}'}, status=status.HTTP_200_OK)
        except Exception as e:
    ...
    def completar_orden_compra(self, request, pk=None):
        ...
        if estado not in ['4', '5']:
            return Response({'error': 'Estado inválido. Debe ser "4" (parcial) o "5" (completa).'}, status=status.HTTP_400_BAD_REQUEST)

        # Obtener los items de la orden
        items_oc = ItemEnOrdenCompra.objects.filter(orden_compra=orden)
        if not items_oc.exists():
            return Response({'error': 'La orden no tiene ítems asociados.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if estado == '4':
            for item in items_data:
                ...
                if not item_oc_id or cantidad is None:
                    return Response({'error': 'Cada item debe tener item_oc_id y cantidad.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for ioc in items_oc:
            ...
            if not item_oc_en_stock:
                return Response({'error': f'No se encontró un registro en ItemOrdenCompraEnStock para el ítem {ioc.id}'}, status=status.HTTP_400_BAD_REQUEST)

            # Obtener la bodega desde bodega_temporal
            bodega = item_oc_en_stock.bodega_temporal
            if not bodega:
                return Response({'error': f'El ítem {ioc.id} no tiene una bodega temporal asociada.'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if cantidad_a_ingresar is None:
                return Response({'error': f'No se proporcionó cantidad parcial para el ítem {ioc.id}'}, status=status.HTTP_400_BAD_REQUEST)

            try:
                cantidad_a_ingresar = int(cantidad_a_ingresar)
            except ValueError:
                return Response({'error': f'La cantidad para el ítem {ioc.id} no es un número válido.'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            stock_item.cantidad += cantidad_a_ingresar
            stock_item.save()
            ...
            registrar_entrada(stock_item=stock_item, cantidad=stock_item.cantidad, usuario=usuario_empresa, origen=item_oc_en_stock, descripcion="Items añadidos desde una orden de compra")

        # Actualizar el estado de la orden
        orden.estado = estado
        orden.save()

        return Response({'message': 'Orden completada con éxito', 'estado': estado}, status=status.HTTP_200_OK)
    ...
    def recientes_por_item(self, request):
        ...
        if not item_id:
            return Response({'error': 'El parámetro "item_id" es obligatorio.'}, status=400)

        try:
            dias = int(dias)
        except ValueError:
            return Response({'error': 'El parámetro "dias" debe ser un número entero.'}, status=400)

        # Obtener la sucursal principal del usuario
        user = self.request.user
        personalizacion = PersonalizacionUsuario.objects.filter(usuario=user).first()
        if not personalizacion or not personalizacion.sucursal_principal:
            return Response({'error': 'No tiene una sucursal principal asignada.'}, status=403)
    ...
    def add_item_no_proveedor(self, request, pk=None):
        ...
        try:
            ...
            return Response({"error": "El item especificado no existe."}, status=status.HTTP_404_NOT_FOUND)

        proveedor = orden_compra.proveedor

        if not proveedor:
            return Response({"error": "La orden de compra no tiene un proveedor asociado."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_pdf(self, request, pk=None):
        ...
        datos_tabla = [["ARTÍCULO", "DESCRIPCIÓN", "CANTIDAD", "PRECIO UNITARIO", "TOTAL"]]
        for item in items:
            datos_tabla.append([
    ...
    def recientes_por_proveedor(self, request):
        ...
        if not proveedor_id:
            return Response({'error': 'El parámetro "proveedor_id" es obligatorio.'}, status=400)
...
class ItemEnOrdenCompraViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return ItemEnOrdenCompra.objects.filter(orden_compra_id=self.kwargs['orden_compra_pk'])
...
class ItemOrdenCompraEnStockViewSet(viewsets.ModelViewSet):
    ...
    def por_orden(self, request, orden_compra_pk=None):
        ...
        content_type_oc = ContentType.objects.get(app_label='bodegas', model='itemenordencompra')

        # Obtener los IDs de los items en la orden de compra específica
        item_en_orden_ids = ItemEnOrdenCompra.objects.filter(orden_compra_id=orden_compra_pk).values_list('id', flat=True)
        ...
        items_oc = self.get_queryset().filter(
            ...
            item_oc_id__in=item_en_orden_ids
        )
...
class GuiaSalidaViewSet(viewsets.ModelViewSet):
    ...
    def aprobar_guia(self, request, pk=None):
        ...
        - Cambia el estado a 'ET' y guarda la firma de quien recibe.
        ...
        try:
            with transaction.atomic():
                ...
                for item_guia in ItemsGuiaSalida.objects.filter(guia=guia_salida).select_related("stock_item"):
                    if item_guia.cantidad_rebajada > item_guia.stock_item.cantidad_no_disponible:
                        return Response(
                            {
                                "detail": (
                ...
                # 2) Rebajar cantidad_no_disponible y registrar salida
                for item_guia in ItemsGuiaSalida.objects.filter(guia=guia_salida):
                    stock_item = item_guia.stock_item
                    item_guia.cantidad_original = stock_item.cantidad
                    ...
                    registrar_salida(
                        ...
                        origen=item_guia,
                        descripcion="Items rebajados desde una guía de salida"
                    )

                # 3) Marcar guía “En Tránsito” y guardar firma
                guia_salida.estado = "ET"
    ...
    def comprobar_guia(self, request, pk=None):
        ...
        try:
            with transaction.atomic():
                # ---------- VALIDACIONES ----------
                for item_guia in guia_salida.itemsguiasalida_set.select_related("stock_item"):
                    if item_guia.cantidad_rebajada > item_guia.stock_item.cantidad_no_disponible:
                        return Response(
                    ...
                    if item_guia.individualizado:
                        ...
                        if not (numero_serie.get("serie") and numero_serie.get("modelo") and numero_serie.get("object_id")):
                            return Response(
            ...
            return Response({"detail": "Uno o más ítems no existen en esta guía de salida."}, status=status.HTTP_404_NOT_FOUND)
    ...
    def devolver_a_bodega(self, request, pk=None):
        ...
        if items_data is None:
            ...
            items_qs = ItemsGuiaSalida.objects.select_for_update().filter(guia=guia, pk__in=ids)
        ...
        try:
            for item in items_qs:
                ...
                if items_data is None:
                    ...
                    data = next((d for d in items_data if int(d.get("item_guia_id")) == item.pk), None)
                    if not data:
                        return Response({"detail": f"Item {item.pk} no enviado correctamente."}, status=status.HTTP_400_BAD_REQUEST)
                    devolver = int(data.get("cantidad_a_devolver", 0))
                    max_dev = item.cantidad_rebajada - item.cantidad_devuelta
                    if devolver > max_dev:
                        return Response({"detail": f"No puedes devolver más de lo rebajado en item {item.pk}."}, status=status.HTTP_400_BAD_REQUEST)

                # 1) Ajustar stock de bodega
                stock = item.stock_item
                stock.cantidad += devolver
                stock.save()

                # 2) Actualizar registro en ItemsGuiaSalida
                item.cantidad_devuelta += devolver
                item.save()

                # 3) Registrar histórico de devolución
                ...
                # 4) Eliminar el Equipo asociado a este número de serie
                serie = item.numero_serie.get("serie")
                if serie:
                    Equipo.objects.filter(numero_serie=serie).delete()

                    # 5) Liberar solo 'modelo' y 'object_id' en ItemOrdenCompraEnStock, sin quitar la serie
                    oc_qs = ItemOrdenCompraEnStock.objects.select_for_update().filter(numeros_serie__icontains=serie)
                    for oc in oc_qs:
                        series_list = oc.numeros_serie.get("numeros_serie", [])
                        for s in series_list:
                            if (
                                s.get("serie") == serie and
                                s.get("modelo") == item._meta.model_name and
                                s.get("object_id") == item.pk
                            ):
                                s["modelo"] = ""
                                s["object_id"] = 0
            ...
            # 6) Recalcular y guardar estado de la guía basada en ItemsGuiaSalida
            ...
            return Response({"detail": "Item(s) no existen en esta guía."}, status=status.HTTP_404_NOT_FOUND)
        except ValueError:
    ...
    def agregar_item(self, request, pk=None):
        ...
        individualizado = request.data.get("individualizado", False)  # Valor por defecto False
        usuario_empresa = obtener_usuario_empresa(request.user)

        # Validar datos de entrada
        if stock_item_id is None or cantidad_rebajada is None:
            return Response({"detail": "Debes proporcionar stock_item_id y cantidad_rebajada."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            cantidad_rebajada = int(cantidad_rebajada)
        except ValueError:
            return Response({"detail": "cantidad_rebajada debe ser un número."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            stock_item = StockItemEnBodega.objects.get(pk=stock_item_id)
        except StockItemEnBodega.DoesNotExist:
            return Response({"detail": "El stock_item_id proporcionado no existe."}, status=status.HTTP_404_NOT_FOUND)

        if cantidad_rebajada > stock_item.cantidad:
            return Response({"detail": "No puedes rebajar más cantidad de la disponible en stock."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            # stock_item.cantidad = stock_item.cantidad - cantidad_rebajada
            stock_item.cantidad_no_disponible = stock_item.cantidad_no_disponible + cantidad_rebajada
            stock_item.cantidad -= cantidad_rebajada
            stock_item.save()

            registrar_salida(stock_item=stock_item, cantidad=stock_item.cantidad, origen=item_guia, usuario=usuario_empresa, descripcion="Items añadidos a la guia")

            serializer = ItemsGuiaSalidaSerializer(item_guia)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        except Exception as e:
            return Response({"detail": f"Error al crear el ítem: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    @action(detail=False, methods=["get"], url_path=r"(?P<empresa_id>[^/.]+)/disponibles")
    ...
    def equipos_guia(self, request, pk=None):
        ...
        seriales = list(
            items.values_list("numero_serie__serie", flat=True)
        )
        ...
        equipos = (
            Equipo.objects.filter(numero_serie__in=seriales, estado=True, cliente__isnull=True)
            .annotate(
                usos_activos=Count(
                    "usuario_equipo",
                    filter=Q(usuario_equipo__estado=True)
                )
            )
...
class ItemsGuiaSalidaViewSet(viewsets.ModelViewSet):
    ...
    def filtrar_por_cliente(self, request):
        ...
        if not cliente_id:
            return Response({"detail": "El parámetro 'cliente_id' es requerido."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def eliminar_item(self, request, pk=None, guia_salida_bodega_pk=None):
        ...
        try:
            ...
            return Response({"detail": "Item de guía no encontrado."},
                            status=status.HTTP_404_NOT_FOUND)
        ...
        stock_item = item_guia.stock_item
        cantidad_rebajada = item_guia.cantidad_rebajada
        stock_item.cantidad += cantidad_rebajada
        stock_item.cantidad_no_disponible = max(0, stock_item.cantidad_no_disponible - cantidad_rebajada)
        stock_item.save()

        registrar_devolucion(stock_item=stock_item, cantidad=stock_item.cantidad, usuario=usuario_empresa, origen=item_guia, descripcion="Items eliminados de una guia de salida")

        # Si el item posee un número de serie en su campo JSON, se resetea en ItemOrdenCompraEnStock
        numero_serie = item_guia.numero_serie  # Estructura simple: { "serie": string, "modelo": string, "object_id": number }
        if numero_serie and numero_serie.get("serie"):
            ...
            for oc in qs_oc:
                ...
                for entry in lista_series:
                    if (entry.get("serie") == serie and 
                        entry.get("modelo") == "itemsguiasalida" and 
                        entry.get("object_id") == item_guia.id):
        ...
        return Response({"detail": "Item eliminado, stock actualizado y serie restablecida correctamente."},
                        status=status.HTTP_200_OK)
    ...
    def editar_item(self, request, pk=None, guia_salida_bodega_pk=None):
        ...
        try:
            ...
            return Response({"detail": "Item de guía no encontrado."}, status=status.HTTP_404_NOT_FOUND)
        ...
        if nueva_cantidad is None:
            return Response({"detail": "El campo 'nueva_cantidad' es requerido."}, status=status.HTTP_400_BAD_REQUEST)
        try:
            nueva_cantidad = int(nueva_cantidad)
        except ValueError:
            return Response({"detail": "El campo 'nueva_cantidad' debe ser un entero."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if delta > 0:
            ...
            stock_item.cantidad -= delta
            stock_item.cantidad_no_disponible += delta
            registrar_salida(stock_item=stock_item, cantidad=stock_item.cantidad, usuario=usuario_empresa, origen=item_guia, descripcion="Items aumentados en la guia de salida")
        # Si se reduce la cantidad, se devuelven ítems al stock
        elif delta < 0:
            stock_item.cantidad += abs(delta)
            stock_item.cantidad_no_disponible = max(0, stock_item.cantidad_no_disponible - abs(delta))
            registrar_entrada(stock_item=stock_item, cantidad=stock_item.cantidad, usuario=usuario_empresa, origen=item_guia, descripcion="Items reducidos en la guia de salida")
    ...
    def actualizar_serie(self, request, pk=None, guia_salida_bodega_pk=None):
        ...
        if not serie:
            return Response({"detail": "El campo 'serie' es requerido."},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        if not serie_actualizada:
            return Response({"detail": "Serie no encontrada en ItemOrdenCompraEnStock."},
                            status=status.HTTP_404_NOT_FOUND)
...
class CompraViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        personalizacion = PersonalizacionUsuario.objects.filter(usuario=user).first()
        if personalizacion and personalizacion.sucursal_principal:
            return Compra.objects.filter(sucursal=personalizacion.sucursal_principal)
        return Compra.objects.none()

    @action(detail=True, methods=['post'], url_path='completar')
    def completar(self, request, pk=None):
        """
        Cambia estado a "1" y transfiere los ItemOrdenCompraEnStock relacionados
        al stock de la bodega (creando o actualizando StockItemEnBodega).
        """
        ...
        with transaction.atomic():
            ...
            # ContentType de ItemEnCompra
            ct = ContentType.objects.get_for_model(ItemEnCompra)
            # Todos los ItemEnCompra de esta compra
            items_compra = ItemEnCompra.objects.filter(compra=compra)

            for item_en_compra in items_compra:
                relaciones = ItemOrdenCompraEnStock.objects.filter(
                    content_type=ct,
                    item_oc_id=item_en_compra.pk
                )

                for rel in relaciones:
                    if not rel.bodega_temporal:
                        return Response(
                            {"detalle": f"Falta bodega_temporal en relación {rel.pk}"},
                            status=status.HTTP_400_BAD_REQUEST
                        )

                    bodega = rel.bodega_temporal
                    articulo = item_en_compra.item

                    # 1) Intentamos actualizar si ya existe stock
                    qs = StockItemEnBodega.objects.filter(
                        item=articulo,
                        bodega=bodega
                    )
                    if qs.exists():
                        # usa F() aquí, con update() en el QuerySet
                        qs.update(cantidad=F('cantidad') + rel.cantidad)
                        # obtener el objeto actualizado
                        stock_item = qs.first()
                        # para que se registre en el histórico (si aplica):
                        stock_item.save(update_fields=['cantidad'])
                    else:
                        # creación "a mano", sin F()
                    ...
                    # 2) enlazamos el stock_item a la relación
                    rel.stock_item = stock_item
                    rel.save()
                    registrar_entrada(stock_item=stock_item, cantidad=stock_item.cantidad, usuario=usuario_empresa, origen=rel, descripcion="Items añadidos desde una compra")
    ...
    def crear_item_empresa(self, request, compras_pk=None):
        ...
        with transaction.atomic():
            ...
            if not item_empresa_data:
                return Response({'error': 'No se han proporcionado datos para ItemEmpresa.'},
                                status=status.HTTP_400_BAD_REQUEST)
            ...
            if item_empresa_serializer.is_valid():
                ...
                return Response(item_empresa_serializer.errors, status=status.HTTP_400_BAD_REQUEST)

            imagenes = request.data.get("imagenes")
            if imagenes:
                for x in imagenes:
                    imagen_serializer = ImagenItemSerializer(data={"imagen": x, "item": nuevo_item_empresa.pk})
            ...
            if item_en_compra_serializer.is_valid():
                ...
                return Response(item_en_compra_serializer.data, status=status.HTTP_201_CREATED)
            else:
                return Response(item_en_compra_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class MovimientoStockViewSet(viewsets.ModelViewSet):
    ...
    def crear_inicial(self, request):
        ...
        return Response(MovimientoStockSerializer(movimiento, context=self.get_serializer_context()).data, status=status.HTTP_201_CREATED)
    ...
    def crear_ajuste(self, request):
        ...
        movimiento = MovimientoStock.objects.create(
            ...
            cantidad=qty_change,
            descripcion=data.get("descripcion", "Item ajustado manualmente en la bodega"),
            usuario=user,
        )
