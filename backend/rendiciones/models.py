...
class ItemRendicion(ModeloBase):
    rendicion = models.ForeignKey(Rendicion, on_delete=models.CASCADE, verbose_name="Rendición", related_name="items")
    opciones = Q(app_label='ordentrabajo', model='detallegastorendicionot') | Q(app_label='rendiciones', model='detallegastorendicion') | Q(app_label='bodegas', model='compra')
