...
from ordentrabajo.models import DetalleGastoRendicionOT
from ordentrabajo.serializers import DetalleGastoRendicionOTSerializer
from .serializers import CategoriaGastoRendicionSerializer, CompraRendicionSerializer, DetalleGastoRendicionSerializer, ItemRendicionSerializer, RendicionSerializer
...
class RendicionViewSet(viewsets.ModelViewSet):
    ...
    def rendiciones_sucursal(self, request):
        ...
        try:
            ...
            if not sucursal:
                return Response({"detail": "Sucursal principal no seleccionada."}, status=400)
    ...
    def descargar_pdf(self, request, pk=None):
        ...
        header = ["Categoría", "Detalle", "Cantidad", "Monto Unitario", "Monto Total", "Fecha Gasto"]
        datos_tabla = [header]
        ...
        fecha_rendicion = rendicion.fecha_rendicion.strftime("%d-%m-%Y") if rendicion.fecha_rendicion else ""
        ...
        response['Content-Disposition'] = f'attachment; filename="rendicion_{id_rendicion}.pdf"'
        return response

    @action(detail=False, methods=['get'], url_path='detalles-ot-libres')
    def detalles_ot_libres(self, request):
        """
        Listado de DetalleGastoRendicionOT que aún no han sido rendidos
        (es decir, no existen en ningún ItemRendicion).
        """
        # obtengo el ContentType para DetalleGastoRendicionOT
        ct_ot = ContentType.objects.get_for_model(DetalleGastoRendicionOT)
        # IDs de DetalleGastoRendicionOT ya usados en algún ItemRendicion
        usados = ItemRendicion.objects.filter(content_type=ct_ot).values_list('detalle_id', flat=True)
        # filtro los que NO estén en esa lista
        libres = DetalleGastoRendicionOT.objects.exclude(pk__in=usados)

        serializer = DetalleGastoRendicionOTSerializer(libres, many=True)
        return Response(serializer.data)
    ...
    def compras_libres(self, request):
        ...
        usados = ItemRendicion.objects.filter(content_type=ct_compra).values_list('detalle_id', flat=True)

        # 3. Filtra las Compras con estado='1' y excluye las ya usadas
        compras_disponibles = Compra.objects.filter(estado='1', sucursal=user.sucursal).exclude(pk__in=usados)
    ...
    def create_item(self, request, rendicion_pk=None):
        ...
        if ct_id:
            ...
            if ct.app_label == 'bodegas' and ct.model == 'compra':
                ...
                return Response(
                    ItemRendicionSerializer(item).data,
                    status=status.HTTP_201_CREATED
                )

            # Gastos OT o internos: uso directo de detalle_id
            if (ct.app_label, ct.model) in {
                ('ordentrabajo', 'detallegastorendicionot'),
