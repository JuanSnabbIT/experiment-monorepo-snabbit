...
const StyledButton: FC<IStyledButtonProps> = ({ text, icon, status }) => {
	...
	const actualizarTema = async (tema: string) => {
		setIsChanging(true)
		try {
			const response = await ApiService.fetchData({url: `/api/personalizacion-usuarios/${personalizacionUsuario?.id}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({tema: tema})})
			if (response.data) {
				dispatch(obtenerPersonalizacionThunk({access}))
				setIsChanging(false)
			}
		} catch (error: any) {
			toast.error(error.response.data, {toastId: "Error en StyledButton al actualizar tema"})
			setIsChanging(false)
		}
	}
