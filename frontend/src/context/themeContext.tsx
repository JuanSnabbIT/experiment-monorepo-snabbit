...
export const ThemeContextProvider: FC<IThemeContextProviderProps> = ({ children }) => {
    ...
    const [asideStatus, setAsideStatus] = useState(() => {
        const stored = localStorage.getItem('fyr_asideStatus');
        // Valor por defecto según localStorage o true
        const defaultStatus = stored ? stored === 'true' : true;
        const mdBreakpoint = Number(theme.screens.md.replace('px', ''));

        // Si estamos en el cliente y el ancho es menor que el breakpoint md, forzamos false
        if (typeof window !== 'undefined' && window.innerWidth < mdBreakpoint) {
            return false;
        }
