...
import { duplicarCotizacionThunk, listaCopiasCotizacionThunk, useAppDispatch, useAppSelector } from '@/store'
...
const CopiasCotizacion = ({
    ...
    useEffect(() => {
        ...
        const cargarCopias = async () => {
            setIsLoading(true)
            try {
                await dispatch(listaCopiasCotizacionThunk({ id_cotizacion: cotizacion.id })).unwrap()
            } catch (error) {
    ...
    const handleCrearCopia = async () => {
        ...
        try {
            ...
            toast.success(`Copia creada N ${nuevaCotizacion.numero_cotizacion}`, { autoClose: 1000 })
            setIsOpen(false)
    ...
    return (
        <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
            <ModalHeader>
                <Badge className="text-xl">Copias de Cotizacion #{cotizacion.numero_cotizacion}</Badge>
            </ModalHeader>
            <ModalBody>
                {isLoading ? (
                        ...
                        <Table className="table-fixed min-w-[500px]">
                            ...
                            <TBody>
                                {copias.map((copia) => (
                                    <Tr key={copia.id}>
                                        <Td>
                                            <span className="font-semibold">#{copia.numero_cotizacion}</span>
                                        </Td>
                                        <Td>{copia.nombre}</Td>
                                        <Td>
                                            <Badge variant="solid" color={getEstadoColor(copia.estado_label)} className="capitalize">
                                                {copia.estado_label}
                                            </Badge>
                                        </Td>
                                        <Td>{new Date(copia.fecha_creacion).toLocaleDateString()}</Td>
                                        <Td>
                                            <Button
                                                variant="solid"
                                                color="violet"
                                                icon="HeroEye"
                                                onClick={() => { navigate(`/cotizacion/detalle-cotizacion/${copia.numero_cotizacion}/`) }}
                                            ></Button>
                                        </Td>
                                    </Tr>
                                ))}
            ...
            <ModalFooter>
                ...
                <ModalFooterChild>
                    ...
                    {puedeCrearCopia ? (
                        <Button variant="solid" onClick={handleCrearCopia}>Crear copia</Button>
                    ) : null}
                </ModalFooterChild>
            </ModalFooter>
        </Modal>
