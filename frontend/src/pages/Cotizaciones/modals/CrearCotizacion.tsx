...
function CrearCotizacion({empresa, onSuccess} : {empresa: boolean, onSuccess?: (cotizacion: any) => void}) {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            ...
            porcentaje_recargo: Yup.number()
                .when("cliente", {
                    is: (cliente: string) => !!cliente,
                    then: (schema) => schema.required("Requerido").min(0, "Debe ser mayor o igual a 0"),
                    otherwise: (schema) => schema.notRequired(),
                })
            ...
            <Modal isOpen={isOpen} setIsOpen={setIsOpen} isStaticBackdrop={true}>
                                ...
                                <SelectReact
                                    ...
                                    onChange={(option: any) => {
                                        const clienteSeleccionado = listaMisClientes.find(
                                            (cliente) => cliente.info_cliente.id.toString() === option?.value
                                        )?.info_cliente;
                ...
                <ModalFooter>
                    ...
                    <ModalFooterChild>
                        ...
                        <Button variant='solid' onClick={() => {formik.handleSubmit()}} isDisable={formik.isSubmitting} isLoading={formik.isSubmitting}>Crear</Button>
                    </ModalFooterChild>
                </ModalFooter>
            </Modal>
        </>
