        ...
        return (
            <div className='flex min-h-screen items-center justify-center bg-gray-100 p-4'>
                <div className='w-full max-w-md rounded-lg bg-white p-8 text-center shadow-lg'>
                    ...
                    <p className='mb-6 text-gray-600'>{error || 'No se pudo cargar la cotización.'}</p>
                    <button
                ...
                <div className='overflow-hidden rounded-xl bg-white shadow-lg print:rounded-none print:shadow-none'>
                    ...
                    <div className='space-y-5 p-6 sm:p-8'>
                                ...
                                <div className='min-w-0 flex-1'>
                                    <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
                                        <div className='min-w-0'>
                                            <p className='text-lg font-semibold text-gray-900'>{cotizacion.empresa?.nombre}</p>
                        ...
                        <div className='rounded-lg border border-gray-200 bg-white p-5'>
                            <div className='flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between'>
                                <div className='min-w-0'>
                                    ...
                                    {cotizacion.nombre && (
                                        <p className='mt-1 text-sm text-gray-500'>{cotizacion.nombre}</p>
                                    )}
                                </div>
                            </div>
                                    ...
                                    <div className='p-4'>
                                        <dl className='divide-y divide-gray-100 text-sm'>
                                            <div className='grid grid-cols-1 gap-1 py-2 sm:grid-cols-[160px,1fr] sm:gap-3'>
                                                ...
                                                <dd className='font-medium text-gray-900'>{cotizacion.cliente?.nombre || 'No especificado'}</dd>
                                            </div>

                                            <div className='grid grid-cols-1 gap-1 py-2 sm:grid-cols-[160px,1fr] sm:gap-3'>
                                                <dt className='text-gray-500'>RUT</dt>
                                                <dd className='font-medium text-gray-900'>{cotizacion.cliente?.rut_empresa || '—'}</dd>
                                            </div>
                                    ...
                                    <div className='p-4'>
                                        <dl className='divide-y divide-gray-100 text-sm'>
                                            <div className='grid grid-cols-1 gap-1 py-2 sm:grid-cols-[160px,1fr] sm:gap-3'>
                                                ...
                                                <dd className='font-medium text-gray-900'>{cotizacion.tipo_moneda_display}</dd>
                                            </div>

                                            <div className='grid grid-cols-1 gap-1 py-2 sm:grid-cols-[160px,1fr] sm:gap-3'>
                                                <dt className='text-gray-500'>Emisión</dt>
                                                <dd className='font-medium text-gray-900'>{formatFechaLarga(cotizacion.fecha_creacion)}</dd>
                                            </div>
                            ...
                            {cotizacion.descripcion && (
                                <div className='mt-4 rounded-md border border-gray-100 bg-gray-50/60 px-4 py-3'>
                                    <p className='text-sm text-gray-700'>
                                        <span className='font-medium text-gray-900'>Descripción:</span> {cotizacion.descripcion}
                                    </p>
                                </div>
                            )}
                        </div>
                            ...
                            <table className='w-full text-sm'>
                                <thead>
                                    <tr className='border-b border-gray-200 bg-gray-50'>
                                        {puedeResponder && (
                                            <th className='w-10 py-3 text-center text-xs font-semibold uppercase tracking-wide text-gray-600'></th>
                                        )}
                                        <th className='py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-600'>
                                            Descripción
                                        </th>
                                        <th className='w-20 py-3 text-center text-xs font-semibold uppercase tracking-wide text-gray-600'>
                                            Cant.
                                        </th>
                                        <th className='w-36 whitespace-nowrap py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-600'>
                                    ...
                                    {currentItems.map((item) => (
                                        <tr
                                            ...
                                            <td className='py-3 pr-4'>
                                                <div className='font-medium text-gray-900'>{item.nombre_display}</div>
                                                {item.descripcion && (
                        ...
                        {totalItems > itemsPerPage && (
                            <div className='mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-3 print:hidden'>
                                ...
                                <div className='flex items-center gap-1 text-sm'>
                                    <button onClick={() => setCurrentPage(1)} disabled={currentPage === 1} className='px-2 py-1 text-gray-500 hover:text-gray-700 disabled:opacity-30'>«</button>
                                    <button onClick={() => setCurrentPage((p) => p - 1)} disabled={currentPage === 1} className='px-2 py-1 text-gray-500 hover:text-gray-700 disabled:opacity-30'>‹</button>
                                    <span className='px-2 text-gray-600'>{currentPage}/{totalPages}</span>
                                    <button onClick={() => setCurrentPage((p) => p + 1)} disabled={currentPage === totalPages} className='px-2 py-1 text-gray-500 hover:text-gray-700 disabled:opacity-30'>›</button>
                                    <button onClick={() => setCurrentPage(totalPages)} disabled={currentPage === totalPages} className='px-2 py-1 text-gray-500 hover:text-gray-700 disabled:opacity-30'>»</button>
                                </div>
                            </div>
                        )}
                        ...
                        <p className='mt-3 text-[11px] italic leading-snug text-gray-400'>{getMonedaDisclaimer()}</p>

                        {cotizacion.observaciones && (
                            <div className='mt-4 border-t border-gray-100 pt-3'>
                                <p className='text-sm text-gray-600'>
                                    <span className='font-medium text-gray-700'>Observaciones:</span> {cotizacion.observaciones}
                                </p>
                            </div>
                        )}
                            </div>
                        ...
                        <div className='border-t border-gray-100 pt-5 text-center'>
                            ...
                            <p className='mt-3 text-sm'>
                                <span className='text-gray-500'>Atentamente,</span>{' '}
                                <span className='font-medium text-gray-700'>{cotizacion.empresa?.nombre}</span>
                            </p>
                        </div>

                        {/* Card: Responder */}
                        {puedeResponder && (
                            <div className='rounded-lg border border-blue-200 bg-blue-50 p-5 print:hidden'>
                                <div className='flex flex-col items-center gap-4 sm:flex-row sm:justify-center'>
                                    ...
                                    <button
                                        ...
                                        disabled={submitting}
                                        className='flex items-center gap-2 rounded-lg border border-red-600 bg-white px-6 py-2.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50'>
                ...
                <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4'>
                    <div className='w-full max-w-md rounded-lg bg-white shadow-xl'>
                        ...
                        <div className='flex items-center justify-between border-b border-gray-200 px-5 py-3'>
                            <h3 className='font-semibold text-gray-900'>Rechazar cotización</h3>
                            <button
