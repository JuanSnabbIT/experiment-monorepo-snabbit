...
function CambiarEstadoAsistenciaUsuario({
    id_visita,
    info,
    tipo,
}: {
    id_visita: number | string | undefined;
    info: IAsistenciaUsuario;
    tipo?: string;
}) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                if (response.data) {
                    toast.success(
                        `Asistencia de usuario ${ESTADO_REVISION_EQUIPO.find((rev) => rev.value === formik.values.estado_revision)?.label}`,
                        { autoClose: 1000 },
                    );
                    if (tipo === '1') {
                ...
                toast.error(error.response.data || 'Error al completar la asistencia de usuario', {
                    toastId: 'Error al completar la asistencia de usuario',
                });
            }
        },
    });
                    ...
                    <div className='flex flex-col gap-4'>
                        <div className='w-full'>
                            ...
                            <SelectReact
                                ...
                                value={ESTADO_REVISION_EQUIPO.find(
                                    (rev) => rev.value === formik.values.estado_revision,
                                )}
                            />
                        </div>
                        <div className='w-full'>
