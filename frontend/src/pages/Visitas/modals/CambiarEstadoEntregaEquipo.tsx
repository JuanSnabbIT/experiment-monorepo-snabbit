...
function CambiarEstadoEntregaEquipo({entrega, tipo} : {entrega: IEntregaEquipo | IEntregaEquipoEnOT, tipo?: string}) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                let data = {observaciones_entrega: values.observaciones_entrega, estado_entrega: values.estado_entrega}
                if (values.estado_entrega === "entregado" || values.estado_entrega === "no_usuario") {
                    Object.assign(data, {firma_entregado: sigCanvas.current?.toDataURL("image/png")})
                }
                if (values.estado_entrega === "no_usuario") {
                    Object.assign(data, {nombre_quien_recibe: values.nombre_quien_recibe})
                }
                const response = await ApiService.fetchData({url: `/api/visitas-soporte/${entrega.visita}/entregas-equipos/${entrega.id}/`, method: "patch", headers: {'Content-Type': 'application/json'}, data: JSON.stringify(data)})
                if (response.data) {
                    if (values.estado_entrega === "entregado" || values.estado_entrega === "no_usuario") {
        ...
        <>
            {entrega.se_puede_firmar ? (
                ...
                <Tooltip text="El equipo necesita más datos">
                    <div>
                        <Button color="sky" variant="solid" icon='HeroArrowPathRoundedSquare' isDisable={true} />
                    </div>
                </Tooltip>
            )}
            <Modal size={"lg"} isOpen={isOpen} setIsOpen={setIsOpen}>
                ...
                <ModalBody>
                    <div className="flex flex-col gap-4">
                        <div className="w-full">
                            ...
                            >
                                <SelectReact
                                    ...
                                    onChange={(e) => {formik.setFieldValue("estado_entrega", (e as TSelectOption).value)}}
                                    value={ESTADO_ENTREGA_EQUIPO.find(ent => ent.value === formik.values.estado_entrega)}
                        ...
                        {(formik.values.estado_entrega === "entregado" || formik.values.estado_entrega === "no_usuario") && (
                            <div className="w-full">
                                <Badge>Firma</Badge>
