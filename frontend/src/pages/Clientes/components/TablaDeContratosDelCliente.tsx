...
import { LIMPIAR_DETALLE_CONTRATO, listaContratosDeEmpresaYClienteThunk } from "@/store/slices/contratos/contratoSlice"
...
function TablaDeContratosDelCliente() {
    ...
    useEffect(() => {
        if (listaContratosDeEmpresaYCliente.length > 0) {
            setOptionsContratos(listaContratosDeEmpresaYCliente.map(cont => ({value: cont.id.toString(), label: cont.nombre})))
        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {listaContratosDeEmpresaYCliente.length > 0 ? (
                        listaContratosDeEmpresaYCliente.map((contrato, index) => (
                            <Card key={index} className="border border-blue-500 rounded-xl">
                                <CardHeader>
                                    <CardHeaderChild>
                                        <Badge className="text-xl">{contrato.nombre}</Badge>
                                    </CardHeaderChild>
                                    <CardHeaderChild>
                                        <Button variant="solid" color="violet" icon="HeroEye" onClick={() => {navigate(`/empresa/contratos-cliente/${detalleCliente?.id}?contrato=${contrato.id}`)}} />
                                        <Tooltip text="PDF">
                                            <Button variant="solid" color="red" icon="HeroDocument" onClick={() => {navigate(`/pdf-contrato/${contrato.id}/12`)}} />
                                        </Tooltip>
                                        <Button variant="solid" color="amber" icon="HeroDocument" onClick={async () => {
                                            try {
                                                const response = await ApiService.fetchData<BlobPart>({url: `/api/contratos/${contrato.id}/pdf/`, method: 'get', headers: {'Content-Type': 'application/pdf'}})
                                ...
                                <CardBody>
                                    <div className="grid grid-cols-1 gap-4">
                                        <div>
                                            ...
                                            - {contrato.fecha_fin ? dayjs(contrato.fecha_fin).format("DD/MM/YYYY") : "Sin Fecha de Finalizacion"}
                                            <Badge className="ml-2" variant="solid" color={contrato.estado === "borrador" ? "amber" : contrato.estado === "activo" ? "emerald" : contrato.estado === "suspendido" || contrato.estado === "finalizado" ? "red" : "zinc"}>{contrato.estado_label}</Badge>
                                        </div>
