...
import { detalleContratoLicenciaThunk, LIMPIAR_DETALLE_LICENCIA, LIMPIAR_USUARIOS_VINCULADOS_LICENCIA, listaContratoLicenciaDeEmpresaYClienteThunk, listaUsuariosVinculadosLicenciaThunk, useAppDispatch, useAppSelector } from "@/store"
...
function TablaDeUsuariosVinculadosLicencias() {
    ...
    const { listaContratoLicenciaDeEmpresaYCliente, listaUsuariosVinculadosLicencia, detalleContratoLicencia } = useAppSelector((state) => state.contrato)
    const [contratoLicencia, setContratoLicencia] = useState<{id: string, nombre: string} | undefined>()
    ...
    return (
        <>
            <Card>
                <CardHeader>
                    <CardHeaderChild>
                        <Badge className="text-xl">Licencias Vinculadas {detalleContratoLicencia && (`${detalleContratoLicencia.nombre_contrato}: ${detalleContratoLicencia.nombre_licencia}`)}</Badge>
                    </CardHeaderChild>
                    <CardHeaderChild>
                        <div>
                        ...
                        <div className="w-[200px]">
                            ...
                            <SelectReact
                                ...
                                onChange={(e) => {
                                    if (e) {
                                        setContratoLicencia({id: (e as TSelectOption).value, nombre: (e as TSelectOption).label})
                                    } else {
                                        setContratoLicencia(undefined)
                                    }
                                }}
                                value={contratoLicencia ? {value: contratoLicencia.id, label: contratoLicencia.nombre} : undefined}
                            />
                        </div>
                    </CardHeaderChild>
                </CardHeader>
                        ...
                        {detalleContratoLicencia && (
                            <>
                                ...
                                <div>
                                    ...
                                    <div className="ml-4">
                                        {detalleContratoLicencia.fecha_inicio_edicion ? dayjs(detalleContratoLicencia.fecha_inicio_edicion).locale("es").format("DD/MM/YYYY") : "Sin Fecha de Inicio"} - {detalleContratoLicencia.fecha_fin_edicion ? dayjs(detalleContratoLicencia.fecha_fin_edicion).locale("es").format("DD/MM/YYYY") : "Sin Fecha de Fin"}
                                    </div>
                                </div>
                                <div>
                                    <Badge>Partner</Badge>
                                    <div className="ml-4">{detalleContratoLicencia.partner ? "Si" : "No"}</div>
                                </div>
                                <div>
                                    <Badge>Dias Restantes de Licencia</Badge>
                                    <div className="ml-4">{detalleContratoLicencia.dias_restantes_licencia}</div>
                                </div>
                            </>
                        )}
                    </div>
                                ...
                                {listaUsuariosVinculadosLicencia.length > 0 ? (
                                    <>
                                        {listaUsuariosVinculadosLicencia.map((user, index) => (
                                            <ItemsTablaDeUsuariosVinculadosLicencias user={user} key={index} />
                                        ))}
                                    </>
                                ) : (
