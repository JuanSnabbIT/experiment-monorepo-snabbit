...
import { listaItemsEmpresaThunk, listaProveedoresEmpresaThunk, listaFabricanteThunk, listaCategoriasThunk } from "@/store/slices/item/itemSlice";
...
const validationSchema = Yup.object({
    nombre: Yup.string().max(250, "El nombre no puede exceder los 250 caracteres").required("El nombre es requerido"),
    descripcion_corta: Yup.string().max(45, "La descripción corta no puede exceder los 45 caracteres").nullable(),
...
function CrearItemEmpresa() {
    ...
    const { listaProveedoresEmpresa, listaFabricante, listaCategorias } = useAppSelector((state) => state.item);
            ...
            try {
                const response = await ApiService.fetchData({
                    ...
                    data: JSON.stringify({
                        ...
                        fabricante: values.fabricante ? parseInt(values.fabricante.toString()) : null,
    ...
    const handleCreateFabricante = async (inputValue: string) => {
        try {
            ...
            const mensajesError = Object.values(error.response.data)
                .flat() // Aplana los arrays en caso de que haya más de uno
                .join(" "); // Une los mensajes en una sola cadena
            toast.error(mensajesError || "Error al crear el fabricante", {toastId: "Error al crear el fabricante"});
        }
    };
    ...
    const handleCreateCategoria = async (inputValue: string) => {
        try {
            ...
            const mensajesError = Object.values(error.response.data)
                .flat() // Aplana los arrays en caso de que haya más de uno
                .join(" "); // Une los mensajes en una sola cadena
            toast.error(mensajesError || "Error al crear la categoría", {toastId: "Error al crear la categoría"});
        }
    };
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div>
                            ...
                            <Validation isValid={formik.isValid} isTouched={formik.touched.descripcion_corta} invalidFeedback={formik.errors.descripcion_corta}>
                                <Input id="descripcion_corta" name="descripcion_corta" value={formik.values.descripcion_corta} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                            </Validation>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <Validation isValid={formik.isValid} isTouched={formik.touched.comentarios} invalidFeedback={formik.errors.comentarios}>
                                <Input id="comentarios" name="comentarios" value={formik.values.comentarios} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                            </Validation>
                        </div>
                        <div>
                            <Badge>Proveedor</Badge>
                            <Validation isValid={formik.isValid} isTouched={!!formik.touched.proveedor} invalidFeedback={Array.isArray(formik.errors.proveedor) ? formik.errors.proveedor.join(", ") : formik.errors.proveedor}>
                                <SelectReact
                                    id="proveedor"
                                    name="proveedor"
                                    ...
                                    onChange={(selectedOptions) => {
                                        formik.setFieldValue("proveedor", Array.isArray(selectedOptions) ? selectedOptions.map((option: any) => (option.value)) : []);
                                    }}
                                    onBlur={formik.handleBlur}
                                    noOptionsMessage={() => "No hay opciones"}
                                />
                        ...
                        <div>
                            {(permissionChecked && hasCameraPermission) && (!escaneado) && (isOpenCamara) && (
                                <Scanner
                                        ...
                                        if (detectedCodes.length > 0) {
                                            for (const code of detectedCodes) {
                                                if (['ean_13', 'code_128', 'code_93', 'code_39', 'upc_a', 'upc_e'].includes(code.format)) {
                                                    setPaused(true);
                                                    formik.setFieldValue("codigo_barras", code.rawValue)
                                                    setPaused(false)
                                                } else {
                            ...
                            {(!isOpenCamara) && (
                                <Button variant="solid" onClick={() => {setIsOpenCamara(true)}}>Mostrar Escaner</Button>
                            )}
                            {!hasCameraPermission && (
