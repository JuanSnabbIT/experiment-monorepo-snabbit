...
import { detalleProveedorEmpresaThunk, listaItemsEmpresaProveedorThunk, listaOrdenesCompraRecientesProveedorThunk } from "@/store/slices/item/itemSlice"
...
const validationSchema = Yup.object({
    ...
    recargo_dolar: Yup.number().required("Requerido").nonNullable("Requerido").min(-1, "No puede ser menor a 0")
});
...
function DetalleProveedorEmpresa() {
    ...
    const { detalleProveedorEmpresa, listaItemsEmpresaProveedor } = useAppSelector((state) => state.item)
    ...
    useEffect(() => {
        if (personalizacionUsuario && personalizacionUsuario.empresa && id) {
            dispatch(detalleProveedorEmpresaThunk({ id_empresa: personalizacionUsuario.empresa, id_proveedor: id }))
            dispatch(listaItemsEmpresaProveedorThunk({ id_empresa: personalizacionUsuario.empresa, id_proveedor: id }))
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                if (response.data) {
                    dispatch(detalleProveedorEmpresaThunk({id_empresa: detalleProveedorEmpresa?.empresa, id_proveedor: detalleProveedorEmpresa?.id}));
                    setIsEditing(false);
                    toast.success("Proveedor editado correctamente", {autoClose: 1000})
                }
            } catch (error: any) {
                const mensajesError = Object.values(error.response.data).flat().join(" ");
                toast.error(mensajesError || "Error al editar el proveedor", {toastId: "Error al editar el proveedor"})
            }
        }
    });
    ...
    const columns = [
        ...
        columnHelper.accessor("datos_categoria.nombre", {
            cell: (info) => (
                <div>{info.row.original.datos_categoria && info.row.original.datos_categoria.nombre || "Sin Categoria"}</div>
            ),
            header: "Categoria"
        }),
        columnHelper.accessor("datos_fabricante.nombre", {
            cell: (info) => (
                <div>{info.row.original.datos_fabricante && info.row.original.datos_fabricante.nombre || "Sin Fabricante"}</div>
            ),
            header: "Fabricante"
        }),
                    ...
                    <Tooltip text="Desasociar item del">
                        <Button variant="solid" color="red"  onClick={async () => {
                            try {
                                ...
                                toast.error(error.response.data || 'Hubo un error al desasociar el item. Por favor, inténtelo de nuevo.', {toastId: 'Hubo un error al desasociar el item. Por favor, inténtelo de nuevo.'});
                            }
                        }} >Desasociar</Button>
                    </Tooltip>
                </div>
    ...
    const columnsOC = [
        ...
        columnHelperOC.display({
            ...
            cell: (info) => (
                <div>
                    <Tooltip text="Detalle">
                        <Button variant="solid" icon="HeroEye" color="violet" onClick={() => {navigate(`/compras/detalle-orden-compra/${info.row.original.id}`)}}></Button>
                    </Tooltip>
                </div>
            ),
        })
    ...
    return (
        <PageWrapper isProtectedRoute={true} name="Detalle Proveedor Empresa" title="Detalle Proveedor Empresa">
            <Subheader>
                <SubheaderLeft>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                                    <div className="w-full">
                                        ...
                                        {isEditing ? (
                                            ...
                                            <div className="">{detalleProveedorEmpresa?.nombre}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        <Badge>Rut</Badge>
                                        {isEditing ? (
                                            ...
                                            <div className="ml-4">{detalleProveedorEmpresa?.rut}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        ...
                                        {isEditing ? (
                                            ...
                                            <div className="ml-4">{detalleProveedorEmpresa?.direccion}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        <Badge>Ejecutivo</Badge>
                                        {isEditing ? (
                                            <Input id="ejecutivo_asignado" name="ejecutivo_asignado" value={formik.values.ejecutivo_asignado} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                                        ) : (
                                            <div className="ml-4">{detalleProveedorEmpresa?.ejecutivo_asignado ? detalleProveedorEmpresa.ejecutivo_asignado : "Sin Ejecutivo"}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        ...
                                        {isEditing ? (
                                            ...
                                            <div className="ml-4">{detalleProveedorEmpresa?.recargo_dolar || "Sin Recargo"}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        <Badge>Correo Ejecutivo</Badge>
                                        {isEditing ? (
                                            <Input id="email_ejecutivo" name="email_ejecutivo" value={formik.values.email_ejecutivo} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                                        ) : (
                                            <div className="ml-4">{detalleProveedorEmpresa?.email_ejecutivo ? detalleProveedorEmpresa.email_ejecutivo : "Sin Correo Ejecutivo"}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        ...
                                        {isEditing ? (
                                            ...
                                            <div className="ml-4">{detalleProveedorEmpresa?.telefono ? detalleProveedorEmpresa.telefono : "Sin Telefono"}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        ...
                                        {isEditing ? (
                                            ...
                                            <div className="ml-4">{detalleProveedorEmpresa?.pagina_web ? detalleProveedorEmpresa.pagina_web : "Sin Pagina"}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        <Badge>Region</Badge>
                                        {isEditing ? (
                                            ...
                                            >
                                                <SelectReact
                                                    ...
                                                    noOptionsMessage={(e) => (`No existe la región ${e.inputValue}`)}
                                                    options={optRegiones}
                                                    onChange={(e) => {formik.setFieldValue('region', (e as TSelectOption).value)}}
                                                    value={optRegiones.find(region => region.value === formik.values.region.toString())}
                                                />

                                            </Validation>
                                        ) : (
                                            <div className="ml-4">{detalleProveedorEmpresa && detalleProveedorEmpresa.region > 0 ? listaRegiones.find(region => region.region_id === detalleProveedorEmpresa.region)?.region_nombre : "Sin Region"}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        ...
                                        {isEditing ? (
                                            ...
                                            >
                                                <SelectReact
                                                    ...
                                                    value={optProvincias.find(provincia => provincia.value === formik.values.provincia.toString())}
                                                />
                                            </Validation>
                                        ) : (
                                            <div className="ml-4">{detalleProveedorEmpresa && detalleProveedorEmpresa.provincia > 0 ? listaProvincias.find(provincia => provincia.provincia_id === detalleProveedorEmpresa.provincia)?.provincia_nombre : "Sin Provincia"}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        <Badge>Comuna</Badge>
                                        {isEditing ? (
                                            ...
                                            >
                                                <SelectReact
                                                    ...
                                                    value={optComunas.find(comuna => comuna.value === formik.values.comuna.toString())}
                                                />
                                            </Validation>
                                        ) : (
                                            <div className="ml-4">{detalleProveedorEmpresa && detalleProveedorEmpresa.comuna > 0 ? listaComunas.find(comuna => comuna.comuna_id === detalleProveedorEmpresa.comuna)?.comuna_nombre : "Sin Comuna"}</div>
                                        )}
                                    </div>
                                    <div className="w-full">
                                        ...
                                        {isEditing ? (
                                            ...
                                            <div className="ml-4">{detalleProveedorEmpresa?.catalogo_web ? detalleProveedorEmpresa.catalogo_web : "Sin Catalogo"}</div>
                                        )}
                                    </div>
                                </div>
                            </form>
                ...
                <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <CardHeaderChild>
                                <AnimacionDeInputModoMovil globalFilter={globalFilter} setGlobalFilter={setGlobalFilter} anchoInput={200}>
                                    {personalizacionUsuario?.empresa && (
                                        <AgregarItem id_empresa={personalizacionUsuario.empresa} id_proveedor={Number(id)} />
                                    )}
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardHeaderChild>
                                <AnimacionDeInputModoMovil globalFilter={globalFilterOC} setGlobalFilter={setGlobalFilterOC} anchoInput={200} />
                            </CardHeaderChild>
                        </CardHeader>
                        <CardBody className="z-0">
                            <div className="overflow-auto">
