...
function ListaProveedoresEmpresa() {
    ...
    const columns = [
        ...
        columnHelper.accessor("ejecutivo_asignado", {
            cell: (info) => (
                <div>{info.row.original.ejecutivo_asignado ? info.row.original.ejecutivo_asignado : "Sin Ejecutivo"}</div>
            ),
            header: "Ejecutivo"
        }),
        columnHelper.accessor("email_ejecutivo", {
            cell: (info) => (
                <div>{info.row.original.email_ejecutivo ? info.row.original.email_ejecutivo : "Sin Correo"}</div>
            ),
            ...
            cell: (info) => (
                <div className='flex gap-2'>
                    <Tooltip text="Detalle Proveedor">
                        <Button variant="solid" icon="HeroEye" color="violet" onClick={() => { navigate(`/registros/detalle-proveedor-empresa/${info.row.original.id}`) }} />
                    </Tooltip>
                    <ModalEliminar
                        mensaje={"¿Está seguro que desea eliminar este proveedor? Se eliminarán todos los registros."}
    ...
    return (
        <PageWrapper isProtectedRoute={true} name="Lista Proveedores Empresa" title="Lista Proveedores Empresa">
            <Subheader>
                <SubheaderLeft>
