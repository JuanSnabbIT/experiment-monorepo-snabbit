...
import { listaContentTypeThunk, listaMovimientosStockThunk, useAppDispatch, useAppSelector } from "@/store"
import TableCardFooterTemplateV2 from "@/templates/Table/TableFooterTemplateV2"
import { createColumnHelper, flexRender, getCoreRowModel, getFilteredRowModel, getPaginationRowModel, getSortedRowModel, RowSelectionState, SortingState, useReactTable } from "@tanstack/react-table"
...
function TablaMovimientosStockEnItem({movSeleccionado} : {movSeleccionado: IMovimientoStock | undefined}) {
    ...
    useEffect(() => {
        if (detalleItemEmpresa && personalizacionUsuario && personalizacionUsuario.empresa) {
            dispatch(listaMovimientosStockThunk({id_empresa: personalizacionUsuario.empresa, id_item: detalleItemEmpresa.id}))
        }
    ...
    const columns = [
        ...
        columnHelper.display({
            ...
            cell: (info) => (
                <div>
                    <Button variant="solid" icon="HeroEye" color={
                        info.row.original.tipo_movimiento === "SALIDA" ? "amber" : 
                        info.row.original.tipo_movimiento === "DEVOLUCION" ? "violet" :
                        info.row.original.tipo_movimiento === "ENTRADA" ? "emerald" :
                        info.row.original.tipo_movimiento === "AJUSTE" ? "zinc" :
                        info.row.original.tipo_movimiento === "INICIAL" ? "sky" : "red"}
                        onClick={() => {
                            let ruta = ""
                            const ct_model = listaContentType.find(ct => ct.id === info.row.original.content_type)?.model
    ...
    useEffect(() => {
        ...
        const rowIndex = listaMovimientosStock.findIndex(
            (m) => m.id === movSeleccionado.id,
        );
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardHeaderChild>
                    {listaMovimientosStock.length === 0 && (
                        <CrearMovimientoStockInicial />
                    )}
                </CardHeaderChild>
            </CardHeader>
                            ...
                            {table.getRowModel().rows.map((row) => (
                                <Tr
                                    ...
                                    className={classNames(
                                        ...
                                        "bg-blue-50 dark:bg-blue-600/20 ring-2 ring-blue-500 ring-inset",
                                    )}
                                >
