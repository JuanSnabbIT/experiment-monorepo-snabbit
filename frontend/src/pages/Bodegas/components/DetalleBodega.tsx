...
function DetalleBodega() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/bodegas/${detalleBodega?.id}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify(values)})
                if (response.data) {
                    dispatch(detalleBodegaThunk({id_bodega: id}))
                    formik.resetForm()
                    setIsEditting(false)
    ...
    const columns = [
        ...
        columnHelper.display({
            ...
            cell: (info) => {
                ...
                return (
                    <div>
                        <Button isDisable={isOpening} variant='solid' icon={num === info.row.original.id.toString() ? "DuoAngleDown" : "DuoAngleUp"} onClick={() => {
                            if (isOpening) return;
            ...
            cell: (info) => (
                <div className="flex flex-wrap gap-2">
                    <Tooltip text="Ir al detalle del item en la empresa">
                        <Button variant="solid" color="violet" icon="HeroEye" onClick={() => {navigate(`/registros/detalle-item-empresa/${info.row.original.item}`)}}></Button>
                    </Tooltip>
    ...
    return (
        <PageWrapper isProtectedRoute={true} title="Detalle Bodega" name="Detalle Bodega">
            <Subheader>
                <SubheaderLeft>
                    <Badge className="text-xl">Detalle de la Bodega</Badge>
                </SubheaderLeft>
                <SubheaderRight className="w-full md:w-auto">
                    <CrearGuiaSalidaEnDetalleBodega isOpen={isOpenSalida} setIsOpen={setIsOpenSalida} id_bodega={detalleBodega?.id} />
                    <Tooltip text="Ver PDF">
                        <Button variant="solid" color="red" icon="HeroDocumentText" onClick={async () => {
                            try {
                                const response = await ApiService.fetchData<BlobPart>({url: `/api/bodegas/${id}/generar-pdf/`, method: 'get', headers: {'Content-Type': 'application/pdf'}})
                    ...
                    <Tooltip text="Ver PDF Resumido">
                        <Button variant="solid" color="red" icon="HeroDocumentText" onClick={async () => {
                            try {
                                const response = await ApiService.fetchData<BlobPart>({url: `/api/bodegas/${id}/generar-pdf-resumido/`, method: 'get', headers: {'Content-Type': 'application/pdf'}})
                                const url = window.URL.createObjectURL(new Blob([response.data]))
                                const a = document.createElement('a');
                                a.href = url;
                            ...
                            <CardHeaderChild>
                                {isEditting ? (
                                    <>
                                        ...
                                        <Button variant="solid" color="red" onClick={() => {setIsEditting(false); formik.resetForm()}}>Cancelar</Button>
                                    </>
                                ) : (
                                    <Button variant="solid" onClick={() => {setIsEditting(true); formik.setFieldValue("nombre", detalleBodega?.nombre)}}>Editar Nombre</Button>
                                )}
                            </CardHeaderChild>
                        </CardHeader>
                        <CardBody>
