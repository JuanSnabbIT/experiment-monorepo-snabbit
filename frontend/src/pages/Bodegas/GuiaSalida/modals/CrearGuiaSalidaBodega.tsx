...
import { useCreateGuiaSalidaMutation } from '@/store/slices/bodega/guiaSalidaApi';
...
function CrearGuiaSalidaBodega() {
    ...
    const [createGuia] = useCreateGuiaSalidaMutation();
    const { personalizacionUsuario, userMe } = useAppSelector((state) => state.auth);
    const { usuarioEmpresaLogeado, listaUsuariosTodaLaEmpresa, listaMisClientes } = useAppSelector(
        (state) => state.empresa,
    );
    ...
    useEffect(() => {
        if (listaBodegas.length > 0) {
            setOptBodegas(
                listaBodegas.map((bod) => {
                    return { value: bod.id.toString(), label: bod.nombre };
                }),
            );
        }
    }, [listaBodegas]);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                const response = await createGuia(payload).unwrap();
                toast.success('Guia de salida de bodega creada', { autoClose: 1000 });
                setIsOpen(false);
                formik.resetForm();
                navigate(`/bodega/detalle-guia-salida-bodega/${response.id}`);
            } catch (error: any) {
                toast.error(error.data || 'Error al crear la guia');
            }
        },
    });
                            ...
                            <Validation
                                ...
                                <SelectReact
                                    ...
                                    value={{
                                        ...
                                        label:
                                            optBodegas.find(
                                                (element) => element.value === formik.values.bodega,
                                            )?.label || '',
                                    }}
                                    noOptionsMessage={(e) => `No existe ${e.inputValue}`}
                                    onBlur={formik.handleBlur}
