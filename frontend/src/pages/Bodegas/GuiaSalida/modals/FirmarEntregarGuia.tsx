...
import { listaGuiaSalidaPorBodegaThunk, useAppDispatch, useAppSelector } from "@/store"
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            if (sigCanvas.current?.isEmpty()) {
                toast.error("Por favor firme la entrega" , {toastId: "Por favor firme la entrega"})
            } else {
                try {
                    const response = await ApiService.fetchData({url: `/api/guia-salida/${id_guia}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                        ...values,
            ...
            <Modal isOpen={isOpen} setIsOpen={setIsOpen} isStaticBackdrop={true}>
                    ...
                    <div className="w-full">
                        ...
                        >
                            <SelectReact
                                ...
                                options={listaUsuariosDeMisClientes.map(user => ({value: user.id.toString(), label: user.nombre_usuario}))}
                                value={{value: formik.values.entregado_a, label: listaUsuariosDeMisClientes.find(user => user.id.toString() === formik.values.entregado_a)?.nombre_usuario || ""}}
                                onChange={(e) => {formik.setFieldValue("entregado_a", (e as TSelectOption).value)}}
                            />
                        </Validation>
                    </div>
                    <div>
                        <Badge>Firma</Badge>
                        <div onBlur={formik.handleBlur} className="dark:bg-white" style={{ width: '100%', maxWidth: '600px', margin: '0 auto' }}>
                            <SignatureCanvas
                ...
                <ModalFooter>
                    ...
                    <ModalFooterChild>
                        ...
                        <Button variant="solid" onClick={async () => {formik.handleSubmit()}}>Aceptar Entrega</Button>
                    </ModalFooterChild>
                </ModalFooter>
            </Modal>
        </>
