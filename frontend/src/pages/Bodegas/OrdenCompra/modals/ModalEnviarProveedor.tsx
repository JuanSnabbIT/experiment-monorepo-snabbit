...
function ModalEnviarProveedor({
    id_empresa,
    id_proveedor,
    id_orden,
    onSuccess,
}: {
    id_empresa: string | number | null | undefined;
    id_proveedor: string | number | undefined | null;
        ...
        <>
            <Tooltip text='Enviar al Proveedor'>
                <Button
                    variant='solid'
                    onClick={() => {
                        dispatch(detalleProveedorEmpresaThunk({ id_empresa, id_proveedor }));
                        setIsOpen(true);
                    }}
                    icon='DuoMail'></Button>
            </Tooltip>
            <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
                ...
                <ModalBody>
                    ...
                    {isTakingLong && (
                        <div className='mt-4 animate-pulse rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800 shadow-sm dark:border-blue-700 dark:bg-blue-900/20 dark:text-blue-200'>
                            <div className='mb-1 font-semibold'>Nota informativa:</div>
                            El proceso de envío está tomando más tiempo de lo habitual debido a la
                            generación del PDF. Si lo prefiere, puede cerrar esta ventana; la
                            operación continuará en segundo plano.
                        </div>
                    )}
                </ModalBody>
                <ModalFooter>
                        ...
                        <Button
                            ...
                            onClick={async () => {
                                ...
                                try {
                                    ...
                                    if (response.data) {
                                        toast.success('Orden de compra cambiada de estado', {
                                            autoClose: 1000,
                                        });
                                        dispatch(
                                            ordenCompraApi.util.invalidateTags([
                                    ...
                                    const mensajesError = errorData
                                        ? typeof errorData === 'object'
                                            ...
                                            : errorData
                                        : error.message || 'Error al pasar de estado la OC';
                                    toast.error(mensajesError, {
                                        toastId: 'error-cambiar-estado-oc',
                                    });
                                } finally {
                                    setIsSubmitting(false);
                                }
                            }}>
                            {isSubmitting ? 'Procesando...' : 'No Enviar al Proveedor'}
                        </Button>
                    </ModalFooterChild>
                    <ModalFooterChild>
                        <Button
