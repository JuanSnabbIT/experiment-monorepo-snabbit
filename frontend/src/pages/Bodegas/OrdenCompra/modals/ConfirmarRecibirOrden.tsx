...
import { IItemEnOrdenCompra, IItemOrdenCompraEnStock, IOrdenCompra } from "@/interface/bodega.interface"
...
function ConfirmarRecibirOrden({itemsARecibir, detalleOrdenCompra} : {itemsARecibir: {item_orden: IItemEnOrdenCompra, item_stock: IItemOrdenCompraEnStock | undefined }[], detalleOrdenCompra: IOrdenCompra | undefined}) {
    ...
    const handleConfirmShow = async () => {
        ...
        let guiaPendienteData: {existe: boolean; guia_id: number | null} = { existe: false, guia_id: null }
        
        // Fetch guia pendiente if conditions allow
        if (detalleOrdenCompra?.id && resul.tiene_cotizacion && resul.tiene_bodega_unica && resul.tiene_items_recibir && resul.bodega_unica_id) {
            try {
        ...
        const puedeCrearGuia = puedeConfirmar && resul.tiene_cotizacion && resul.tiene_bodega_unica && resul.tiene_items_recibir
        ...
        if (resul.itemsConCantidadesDesiguales.length > 0) {
            htmlContent += '<p class="font-bold text-amber-600 mb-1">Items que se recibirán de manera parcial:</p>'
        ...
        if (!resul.tiene_fecha) {
            htmlContent += '<p class="text-red-500">• Falta ingresar la <b>Fecha de Compra</b> en el panel lateral.</p>'
        }
        
        if (!resul.tiene_items_recibir) {
            htmlContent += '<p class="text-amber-500">• No hay ítems con cantidad mayor a 0 para recibir.</p>'
        }

        if (puedeConfirmar && !puedeCrearGuia) {
            htmlContent += '<p class="font-bold mt-4 text-zinc-700">Restricciones para generar Guía de Despacho:</p>'
            if (!resul.tiene_cotizacion) htmlContent += '<p class="text-xs text-zinc-500">• La orden no tiene una cotización asociada.</p>'
            if (!resul.tiene_bodega_unica) htmlContent += '<p class="text-xs text-zinc-500">• Se requiere una sola bodega de recepción para la guía.</p>'
        }

        if (puedeConfirmar && resul.itemsConCantidadesDesiguales.length === 0) {
            htmlContent += '<p class="mt-4 text-zinc-600 italic">Al confirmar la recepción total, la orden pasará a estado finalizado.</p>'
        }
        
        htmlContent += '</div>'

        const labelCrearGuia = guiaPendienteData.existe ? "Agregar a Guía Existente" : "Guardar y Crear Guía"
