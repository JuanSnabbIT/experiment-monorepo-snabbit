...
function EditarItemOrdenStock({item_orden, item_stock, id_orden} : {item_orden: IItemEnOrdenCompra, item_stock: IItemOrdenCompraEnStock | undefined, id_orden: string | number | undefined}) {
    ...
    const validationSchema = Yup.object({
        ...
        cantidad: Yup.number().required("Requerido").nonNullable("Requerido").max(item_orden.cantidad, "No puede superar la cantidad original").min(0, "No puede ser menor a 0"),
    });
    ...
    useEffect(() => {
        if (listaBodegasPorEmpresa.length > 0) {
            setOptionsBodega(listaBodegasPorEmpresa.map(bodega => {return {value: bodega.id.toString(), label: bodega.nombre}}))
        }
    ...
    return (
        <>
            <Tooltip text="Editar Item">
                <Button variant="solid" onClick={() => {setIsOpen(true)}}>Editar</Button>
            </Tooltip>
                ...
                <ModalBody>
                    <div className="w-full grid md:grid-cols-2 grid-cols-1 gap-4">
                        <div className="w-full">
                            ...
                            <div className="flex flex-col gap-2">
                                {formik.values.inputs.map((input, index) => (
                                    <Validation
                                        ...
                                        isTouched={formik.touched.inputs && formik.touched.inputs[index] && formik.touched.inputs[index].value}
                                        invalidFeedback={
                                            ...
                                            (formik.errors.inputs[index] as FormikErrors<InputItem>).value : ""}
                                        key={index}
                                    >
                                        <FieldWrap lastSuffix={
                                            <Button color="red" variant="solid" size="sm" icon="HeroTrash" onClick={() => {
                                                const updatedInputs = formik.values.inputs.filter((_, i) => i !== index); formik.setFieldValue("inputs", updatedInputs)
                                            }}></Button>
                                        }>
                                            <Input
                                                type="text"
                            ...
                            <Button className="mt-2" variant="solid" onClick={() => {formik.setFieldValue("inputs", [...formik.values.inputs, { value: "" }])}}>Añadir N° de Serie</Button>
                        </div>
                        <div className="w-full">
                            <Badge>Bodega</Badge>
                            <Validation
