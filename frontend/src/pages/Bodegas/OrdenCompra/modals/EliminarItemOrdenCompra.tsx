...
function EliminarItemOrdenCompra({id_item, id_orden} : {id_item: number, id_orden: string | number | undefined}) {
        ...
        if (result.isConfirmed) {
            try {
                ...
                if (response.status === 204) {
                    ...
                    dispatch(ordenCompraApi.util.invalidateTags([{ type: 'OrdenCompra', id: id_orden }, { type: 'OrdenCompraItems', id: id_orden }]))
                }
            } catch (error: unknown) {
                const errorMessage = getErrorMessage(error) || "Error al eliminar el item"
                toast.error(typeof errorMessage === 'string' ? errorMessage : "Error al eliminar el item", {
