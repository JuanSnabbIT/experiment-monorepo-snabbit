...
function ListaMisOrdenesCompra() {
    ...
    const {
        ...
        isLoading,
        refetch,
    } = useGetMisOrdenesCompraQuery(undefined, {
                    ...
                    {info.row.original.estado === '-' && (
                        <Tooltip text='Agregar Items'>
                            <Button
                                variant='solid'
                                onClick={() => {
                                    navigate(`/compras/agregar-items-oc/${info.row.original.id}`);
                                }}
                    ...
                    {String(info.row.original.estado) === '1' && (
                        <>
                            <ModalEnviarProveedor
                                id_empresa={info.row.original.oc_empresa}
                                id_proveedor={info.row.original.proveedor}
                            ...
                            <Tooltip text='Recibir Items de la compra'>
                                <Button
                                    ...
                                    onClick={() => {
                                        navigate(
                                            `/compras/completar-orden-compra/${info.row.original.id}`,
                                        );
                                    }}></Button>
                            </Tooltip>
                        </>
                        ...
                        <>
                            ...
                            {useAuthority(listaGrupos?.grupos, ['staff']) && (
                                <ModalVolverABorradorOC
                                    id_orden={info.row.original.id}
                                    disabled={String(info.row.original.estado) !== '0'}
                                />
                            )}
                        </>
                    )}
