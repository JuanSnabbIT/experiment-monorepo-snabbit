...
function CompletarOrdenDeCompra() {
    ...
    const shouldFetchItems = Boolean(
        id &&
            detalleOrdenCompra &&
    ...
    const { data: dataStock, isLoading: isLoadingStock } = useGetItemsOrdenCompraEnStockQuery(
        id || '',
        {
            skip: !shouldFetchItems,
            refetchOnMountOrArgChange: true,
    ...
    const columns = [
        ...
        columnHelper.display({
            id: 'bodega',
            cell: (info) => <div>{info.row.original.item_stock?.nombre_bodega}</div>,
            header: 'Bodega',
        }),
        columnHelper.display({
            id: 'item_stock',
                            ...
                            <CardBody>
                                <div className='grid w-full grid-cols-2 lg:grid-cols-4'>
                                    ...
                                    <div className='w-full'>
                                        <Badge>Cliente</Badge>
                                        <div className='ml-4'>
                                            {detalleOrdenCompra.nombre_cliente}
                                        </div>
                                    </div>
                                    <div className='w-full'>
                                    ...
                                    <div className='w-full'>
                                        <Badge>Estado</Badge>
                                        <div className='ml-4'>
                                            {detalleOrdenCompra.estado_label}
                                        </div>
                                    </div>
                                </div>
                            </CardBody>
                                                        ...
                                                        try {
                                                            if (dolarManual) {
                                                                if (dolarObservado > 0) {
                                                                    ...
                                                                    toast.error(
                                                                        'Los dolares no pueden ser menor a 0',
                                                                    );
                                                                }
                                                            } else {
                                                                const responseDolar =
                                                                    await ApiService.fetchData<IIndicadorDolar>(
                                                                        {
                                                                            baseURL:
                                                                                'https://mindicador.cl/',
                                                                            url: `api/dolar/${dayjs(fechaCompra).format('DD-MM-YYYY')}`,
                                                                            method: 'get',
                                                                            isLoginRequest: true,
                                                                        },
                                                                    );
                                                                        ...
                                                                        await ApiService.fetchData({
                                                                            ...
                                                                            data: JSON.stringify({
                                                                                fecha_compra:
                                                                                    fechaCompra,
                                                                                dolar_observado:
                                                                                    responseDolar.data.serie[0].valor.toFixed(
                                                                                        0,
                                                                                    ) || 0,
                                                                            }),
                                                                        });
