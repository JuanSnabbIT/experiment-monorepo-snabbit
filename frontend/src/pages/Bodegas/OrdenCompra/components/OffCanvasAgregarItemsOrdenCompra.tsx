...
function OffCanvasAgregarItemsOrdenCompra({
    id_orden,
    detalleOrdenCompra,
}: {
    ...
    const { listaItemsEmpresaProveedor, listaCategorias, listaFabricante } = useAppSelector(
        (state) => state.item,
    );
        ...
        if (listaFabricante.length > 0) {
            setOptionFabricante(
                listaFabricante.map((fab) => {
                    return { value: fab.id.toString(), label: fab.nombre };
                }),
            );
        }
    }, [listaFabricante]);
        ...
        if (listaCategorias.length > 0) {
            setOptionCategoria(
                listaCategorias.map((cat) => {
                    return { value: cat.id.toString(), label: cat.nombre };
                }),
            );
        }
    }, [listaCategorias]);
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                if (isCreatingCategoria) {
                    const responseCategoria = await ApiService.fetchData<ICategoria, string>({
                        url: `/api/categorias/`,
                        method: 'post',
                        headers: { 'Content-Type': 'application/json' },
                        data: JSON.stringify({ nombre: formik.values.categoria }),
                    });
                    if (responseCategoria.data) {
                        data = { ...data, categoria: responseCategoria.data.id.toString() };
                    } else {
                        toast.error(responseCategoria.data || 'Error al crear la categoria', {
                            toastId: 'Error al crear la categoria',
                        });
                    }
                }
                if (isCreatingFabricante) {
                    const responseFabricante = await ApiService.fetchData<IFabricante, string>({
                        url: `/api/fabricantes/`,
                        method: 'post',
                        headers: { 'Content-Type': 'application/json' },
                        data: JSON.stringify({ nombre: formik.values.fabricante }),
                    });
                    if (responseFabricante.data) {
                        data = { ...data, fabricante: responseFabricante.data.id.toString() };
                    } else {
                        toast.error(responseFabricante.data || 'Error al crear el fabricante', {
                ...
                if (isCreating) {
                    ...
                    if (response.data) {
                        const responseAgregar = await ApiService.fetchData<any, string>({
                            url: `/api/ordenes-compra/${detalleOrdenCompra?.id}/add_item/`,
                            headers: { 'Content-Type': 'application/json' },
                            method: 'post',
                            data: JSON.stringify({
                        ...
                        if (responseAgregar.data) {
                            ...
                            toast.error(
                                responseAgregar.data.error ||
                                    'Error al agregar el item a la orden de compra',
                                { toastId: 'Error al agregar el item a la orden de compra' },
                            );
                        }
                    } else {
                        toast.error(response.data || 'Error al crear el item en la empresa', {
                            toastId: 'Error al crear el item en la empresa',
                        });
                    }
                } else {
                    const itemProv = listaItemsEmpresaProveedor.find(
                        (it) => data.item === it.id.toString(),
                    );
                    if (itemProv) {
                        const responseAgregar = await ApiService.fetchData<any, string>({
                            url: `/api/ordenes-compra/${detalleOrdenCompra?.id}/add_item/`,
                ...
                toast.error(
                    typeof errorMessage === 'string' ? errorMessage : 'Error al agregar el item',
                    {
    ...
    useEffect(() => {
        if (!isOpen) {
            dispatch(
                ordenCompraApi.util.invalidateTags([{ type: 'OrdenCompraItems', id: id_orden }]),
            );
            setItemSelected(undefined);
            setCategoriaSelected(undefined);
            setFabricanteSelected(undefined);
                            ...
                            <Validation
                                ...
                                <SelectReact
                                    ...
                                    value={
                                        isCreating
                                            ...
                                            : {
                                                  ...
                                                  label:
                                                      listaItemsEmpresaProveedor.find(
                                                          (it) =>
                                                              it.id.toString() ===
                                                              formik.values.item,
                                                      )?.nombre || '',
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setIsCreating(false);
                                        const itemProv = listaItemsEmpresaProveedor.find(
                                            (it) => it.id.toString() === (e as TSelectOption).value,
                                        );
                                        if (itemProv) {
                                            setItemSelected(itemProv);
                                            if (itemProv.categoria) {
                                                setCategoriaSelected({
                                                    value: itemProv.categoria.toString() || '',
                                                    label: itemProv.datos_categoria?.nombre || '',
                                                });
                                                formik.setFieldValue(
                                                    'categoria',
                                                    itemProv.categoria.toString(),
                                                );
                                            } else {
                                                setCategoriaSelected(undefined);
                                                formik.setFieldValue('categoria', '');
                                            ...
                                            if (itemProv.fabricante) {
                                                setFabricanteSelected({
                                                    value: itemProv.fabricante.toString(),
                                                    label: itemProv.datos_fabricante?.nombre || '',
                                                });
                                                formik.setFieldValue(
                                                    'fabricante',
                                                    itemProv.fabricante.toString(),
                                                );
                                            } else {
                                                setFabricanteSelected(undefined);
                                                formik.setFieldValue('fabricante', '');
                            ...
                            <>
                                        ...
                                        <Input
                                            ...
                                            value={
                                                isCreating
                                                    ...
                                                    : itemSelected?.descripcion_corta ||
                                                      'Sin Descripción'
                                            }
                                ...
                                <div className='w-full'>
                                    ...
                                    <SelectReact
                                        ...
                                        onCreateOption={(e) => {
                                            formik.setFieldValue('fabricante', e);
                                        ...
                                        onChange={(e) => {
                                            formik.setFieldValue(
                                                'fabricante',
                                                e ? (e as TSelectOption).value : null,
                                            );
                                            setIsCreatingFabricante(false);
                                            setFabricanteSelected(e as TSelectOption);
                                        }}
                                        value={
                                            isCreatingFabricante
                                                ? {
                                                      value: formik.values.fabricante,
                                                      label: formik.values.fabricante,
                                                  }
                                                : {
                                                      value: formik.values.fabricante,
                                                      label:
                                                          fabricanteSelected?.label ||
                                                          'Sin Fabricante',
                                                  }
                                        }
                                ...
                                <div className='w-full'>
                                    ...
                                    <SelectReact
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setIsCreatingCategoria(false);
                                            setCategoriaSelected(e as TSelectOption);
                                        }}
                                        value={
                                            isCreatingCategoria
                                                ? {
                                                      value: formik.values.categoria,
                                                      label: formik.values.categoria,
                                                  }
                                                : {
                                                      value: formik.values.categoria,
                                                      label:
                                                          categoriaSelected?.label ||
                                                          'Sin Categoria',
                                                  }
                                        }
