...
import { detalleTomaInventarioThunk, listaItemsEnTomaInventarioThunk, useAppDispatch, useAppSelector } from "@/store"
        ...
        <PageWrapper isProtectedRoute={true} name="Inventariar" title="Inventariar">
            <Subheader>
                <SubheaderLeft>
                    ...
                    <Badge className="text-xl">Inventariar Toma N°{detalleTomaInventario?.id}</Badge>
                </SubheaderLeft>
                <SubheaderRight>
                    {detalleTomaInventario && (
                        <>
                            {(detalleTomaInventario.ultimo_estado?.estado === "pendiente" || detalleTomaInventario.ultimo_estado?.estado === "pausado") && (
            ...
            <Container className="w-full h-[calc(100dvh-theme(spacing.20))]">
                ...
                <div className="grid grid-rows-2 lg:grid-rows-none lg:grid-cols-3 gap-4 h-full">
                    <Card className="flex flex-col h-full">
                        <CardBody className="flex-1 min-h-0">
                            {detalleTomaInventario && detalleTomaInventario.ultimo_estado?.estado === "en_proceso" && (
                                <>
                            ...
                            {listaItemsEnTomaInventario.length > 0 && (
                                <>
                                    ...
                                    {listaItemsEnTomaInventario.map((item, index) => (
                                        <div className="grid grid-cols-12 border border-blue-500 rounded-xl min-w-[700px]" key={index}>
                                            <div className="col-span-4 border-r border-r-blue-500 p-2 flex flex-col">
                                                <div>{item.nombre_item}</div>
                                                <div className="text-sm">Bodega: {item.nombre_bodega}</div>
                                            </div>
                                            <div className="col-span-2 border-r border-r-blue-500 p-2">{item.cantidad_original}</div>
                                            <div className="col-span-2 border-r border-r-blue-500 p-2">{item.cantidad_encontrada}</div>
                                            <div className="col-span-2 border-r border-r-blue-500 p-2">{item.estado_label}</div>
                                            <div className="col-span-2 p-2 flex items-center">
                                                {(detalleTomaInventario?.ultimo_estado?.estado === "en_proceso" && item.estado === "por_inventariar") && (
                    ...
                    <ModalConfirmarEscaneoItem isOpen={escaneado} setIsOpen={setEscaneado} codigos={codigos} setCodigos={setCodigos} paused={paused} setPaused={setPaused} />
                </div>
            </Container>
        </PageWrapper>
    )
