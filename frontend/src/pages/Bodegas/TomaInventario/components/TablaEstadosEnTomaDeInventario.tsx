...
function TablaEstadosEnTomaDeInventario() {
    ...
    const { detalleTomaInventario, listaEstadosTomaInventario } = useAppSelector((state) => state.bodega)
    ...
    const columns = [
        ...
        columnHelper.accessor("fecha_cambio", {
            cell: (info) => (
                <div>{info.getValue() ? dayjs(info.getValue()).locale("es").format("DD/MM/YYYY HH:mm:ss") : "Fecha Invalida"}</div>
            ),
            header: "Fecha de Cambio"
        }),
        columnHelper.accessor("observaciones", {
            cell: (info) => (
                <div>{info.getValue() || "Sin Observaciones"}</div>
            ),
