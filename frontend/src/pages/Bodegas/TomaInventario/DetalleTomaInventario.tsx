...
function DetalleTomaInventario() {
    ...
    return (
        <PageWrapper isProtectedRoute={true} name="Detalle Toma de Inventario" title="Detalle Toma de Inventario">
            <Subheader>
                <SubheaderLeft>
                ...
                <SubheaderRight>
                    <Tooltip text="Inventariar">
                        <Button variant="solid" color="sky" icon="DuoIncomingBox" onClick={() => {navigate(`/bodega/inventariar-toma-inventario/${id}`)}}></Button>
                    </Tooltip>
                </SubheaderRight>
            </Subheader>
                    ...
                    {detalleTomaInventario && (
                        <>
                                        ...
                                        <div>
                                            ...
                                            <div className="ml-4">
                                                {detalleTomaInventario.datos_bodegas.length > 0 ? (
                                                    detalleTomaInventario.datos_bodegas.map(bode => bode.nombre).join(", ")
                                                ) : ("Sin Bodegas")}
                                            </div>
                                        </div>
                                        <div>
                                            <Badge>Creado Por</Badge>
                                            <div className="ml-4">{detalleTomaInventario.nombre_creado_por}</div>
                                        </div>
                                        <div>
                                            <Badge>Fecha Inicio</Badge>
                                            <div className="ml-4">{detalleTomaInventario.fecha_inicio ? dayjs(detalleTomaInventario.fecha_inicio).locale("es").format("DD/MM/YYYY HH:mm:ss") : "Sin Fecha de Inicio"}</div>
                                        </div>
                                        <div>
                                            <Badge>Fecha de Termino</Badge>
                                            <div className="ml-4">{detalleTomaInventario.fecha_termino ? dayjs(detalleTomaInventario.fecha_termino).locale("es").format("DD/MM/YYYY HH:mm:ss") : "Sin Fecha de Termino"}</div>
                                        </div>
                                        <div className="col-span-full">
                                            <Badge>Motivo</Badge>
                                            <div className="ml-4">{detalleTomaInventario.motivo}</div>
                                        </div>
                                    </div>
                                </CardBody>
                            </Card>
                            ...
                            <Card>
                                <CardBody>
                                    <div className="flex flex-row gap-4 overflow-auto">
                                        ...
                                        <Button
                                            {...(activeComponent === "Estados"  ? {size: 'sm', rounded: 'rounded-full', className: 'border', isActive: true,color: 'blue', colorIntensity: '500', variant: 'solid',} : {size: 'sm', color: 'zinc', rounded: 'rounded-full', className: 'border'})}
                                            onClick={() => {setActiveComponent("Estados")}}>
                                            Estados
                                        </Button>
                                    </div>
                                </CardBody>
                            ...
                            {activeComponent === "Items" && (
                                <TablaItemsEnTomaDeInventario />
                            )}

                            {activeComponent === "Estados" && (
                                <TablaEstadosEnTomaDeInventario />
                            )}
                        </>
                    )}
                </div>
