...
import { detalleTomaInventarioThunk, useAppDispatch, useAppSelector, usuarioEmpresaLogeadoThunk } from "@/store"
...
function IniciarInventariado({toma} : {toma: ITomaInventario}) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/tomas-inventario/${toma.id}/`, method: "patch", headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                    fecha_inicio: values.fecha_inicio
                })})
                if (response.data) {
                    const responseEstado = await ApiService.fetchData({url: `/api/estados-toma-inventario/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
    ...
    return (
        <>
            <Tooltip text="Iniciar Inventariado">
                <Button variant="solid" color="emerald" icon="HeroPlay" onClick={() => {setIsOpen(true)}} />
            </Tooltip>
            <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
                ...
                <ModalFooter>
                    ...
                    <ModalFooterChild>
                        ...
                        <Button variant="solid" color="emerald" onClick={() => {formik.handleSubmit()}}>Iniciar</Button>
                    </ModalFooterChild>
                </ModalFooter>
            </Modal>
        </>
