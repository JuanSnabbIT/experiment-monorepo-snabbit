...
function CerrarTomaInventario({toma} : {toma: ITomaInventario}) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/tomas-inventario/${toma.id}/cerrar/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify(values)})
                if (response.data) {
                ...
                toast.error(mensajesError || "Error al cerrar el inventariado", {toastId: "Error al cerrar el inventariado"})
            }
        }
    })
    ...
    return (
        <>
            <Tooltip text="Cerrar Inventario">
                <Button variant="solid" color="red" icon="DuoArchive" onClick={() => {setIsOpen(true)}}></Button>
            </Tooltip>
            <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
                ...
                <ModalFooter>
                    ...
                    <ModalFooterChild>
                        ...
                        <Button variant="solid" color="red" onClick={async () => {formik.handleSubmit()}}>Cerrar Inventario</Button>
                    </ModalFooterChild>
                </ModalFooter>
            </Modal>
        </>
