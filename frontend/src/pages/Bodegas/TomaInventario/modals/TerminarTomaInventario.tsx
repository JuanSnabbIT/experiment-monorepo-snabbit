...
import { detalleTomaInventarioThunk, listaItemsEnTomaInventarioThunk, useAppDispatch, useAppSelector, usuarioEmpresaLogeadoThunk } from "@/store"
...
function TerminarTomaInventario({toma} : {toma: ITomaInventario}) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/estados-toma-inventario/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                ...
                toast.error(mensajesError || "Error al iniciar el inventariado", {toastId: "Error al iniciar el inventariado"})
            }
        }
    })
    ...
    return (
        <>
            <Tooltip text="Terminar Inventariado">
                <Button variant="solid" color="red" icon="HeroNoSymbol" onClick={() => {setIsOpen(true)}} />
            </Tooltip>
            <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
                ...
                <ModalBody>
                    <div className="flex flex-col gap-4">
                        {listaItemsEnTomaInventario.filter(item => item.estado === "por_inventariar").length > 0 ? (
                            <>
                                <div>
                                    <Badge className="text-xl">Items Sin Inventariar</Badge>
                                </div>
                                {listaItemsEnTomaInventario.filter(item => item.estado === "por_inventariar").map((item, index) => (
                ...
                <ModalFooter>
                    ...
                    <ModalFooterChild>
                        ...
                        <Button variant="solid" color="red" isDisable={listaItemsEnTomaInventario.filter(item => item.estado === "por_inventariar").length > 0} onClick={() => {formik.handleSubmit()}}>Terminar</Button>
                    </ModalFooterChild>
                </ModalFooter>
            </Modal>
        </>
