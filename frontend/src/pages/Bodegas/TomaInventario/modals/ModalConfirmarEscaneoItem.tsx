...
import { buscarItemEnTomaInventarioThunk, listaItemsEnTomaInventarioThunk, useAppDispatch, useAppSelector } from "@/store"
...
function ModalConfirmarEscaneoItem({isOpen, setIsOpen, codigos, setCodigos, setPaused} : {
    isOpen: boolean,
    ...
    const { detalleTomaInventario, buscarItemEnTomaInventario } = useAppSelector((state) => state.bodega)
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            cantidad_encontrada: Yup.number().required("Requerido").nonNullable("Requerido").min(-1, "Minimo 0"),
            observaciones: Yup.string().required("Requerido").nonNullable("Requerido"),
        }),
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/items-en-toma-inventario/${buscarItemEnTomaInventario[0].id}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                ...
                if (response.data) {
                    if (values.imagenes.length > 0) {
                        const responseImagenes = await ApiService.fetchData({url: `/api/items-en-toma-inventario/${buscarItemEnTomaInventario[0].id}/imagenes/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                ...
                toast.error(mensajesError || "Error al cerrar el inventario del item con observaciones", {toastId: "Error al cerrar el inventario del item con observaciones"})
            }
        }
    })
    ...
    return (
        <>
            <Modal isOpen={isOpen} setIsOpen={setIsOpen} isStaticBackdrop={true}>
                <ModalHeader>
                    <Badge className="text-xl">Confimar Inventario de {codigos.length > 0 ? codigos[0].rawValue : "Sin Codigo"}</Badge>
                </ModalHeader>
                <ModalBody>
                            ...
                            <>
                                {buscarItemEnTomaInventario[0].estado === "por_inventariar" && (
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <Badge>Nombre</Badge>
                                            <div className="ml-4">{buscarItemEnTomaInventario[0].nombre_item}</div>
                                        </div>
                                        <div>
                                            <Badge>Cantidad Original</Badge>
                                            <div className="ml-4">{buscarItemEnTomaInventario[0].cantidad_original}</div>
                                        </div>
                                        {cerrarObservaciones ? (
                                            <>
                                                <div>
                                                ...
                                                <div className="col-span-full">
                                                    {abrirCamara ? (
                                                        ...
                                                        <>
                                                            <Button variant="solid" onClick={() => {setAbrirCamara(true)}}>Abrir Camara</Button>
                                                        </>
                                                    )}
                                                </div>
                                            </>
                                            ...
                                            <div className="flex flex-row gap-4 col-span-full">
                                                <Button variant="solid" color="red" onClick={() => {setCerrarObservaciones(true)}}>Añadir Observaciones</Button>
                                                <Button variant="solid" color="emerald" onClick={async () => {
                                                    try {
                                                        const response = await ApiService.fetchData({url: `/api/items-en-toma-inventario/${buscarItemEnTomaInventario[0].id}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                                                            cantidad_encontrada: buscarItemEnTomaInventario[0].cantidad_original,
                                                            estado: 'inventariado'
                                                        ...
                                                        const mensajesError = Object.values(error.response.data).flat().join(" ");
                                                        toast.error(mensajesError || "Error al cerrar el inventario del item", {toastId: "Error al cerrar el inventario del item"})
                                                    }
                                                }}>Confirmar Inventario</Button>
                                            </div>
                                        )}
                                    </div>
                                )}
                                {(buscarItemEnTomaInventario[0].estado === "inventariado" || buscarItemEnTomaInventario[0].estado === "inventariado_observaciones") && (
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="col-span-full">
                ...
                <ModalFooter>
                    ...
                    <ModalFooterChild>
                        {cerrarObservaciones ? (
                            <Button variant="solid" onClick={() => {formik.handleSubmit()}}>Cerrar Inventario con Observaciones</Button>
                        ) : (
