...
function CrearTomaInventario({
    bodegasSeleccionada,
    fechaInicio,
    fechaTermino,
}: {
    bodegasSeleccionada?: { id: number; nombre: string }[];
    fechaInicio?: string;
    fechaTermino?: string;
}) {
    ...
    const formik = useFormik<IFormikTomaInventario>({
        ...
        validationSchema: Yup.object().shape({
            ...
            motivo: Yup.string().required('Requerido').nonNullable('Requerido'),
        }),
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({
                    url: `/api/tomas-inventario/crear/`,
                    method: 'post',
                        ...
                        <div>
                            ...
                            <Validation
                                ...
                                <SelectReact
                                    ...
                                    value={
                                        formik.values.bodegas.length > 0
                                            ? formik.values.bodegas.map((bode) => ({
                                                  value: bode.id.toString(),
                                                  label: bode.nombre,
                                              }))
                                            : []
