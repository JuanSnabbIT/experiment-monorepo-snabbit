...
function ModalConfirmarItem({item} : {item: IItemEnTomaInventario}) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/items-en-toma-inventario/${item.id}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                ...
                if (response.data) {
                    if (values.imagenes.length > 0) {
                        const responseImagenes = await ApiService.fetchData({url: `/api/items-en-toma-inventario/${item.id}/imagenes/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
    ...
    return (
        <>
            <Tooltip text="Confirmar Inventario">
                <Button color="sky" variant="solid" icon="HeroBookmark" onClick={() => {setIsOpen(true)}} />
            </Tooltip>
