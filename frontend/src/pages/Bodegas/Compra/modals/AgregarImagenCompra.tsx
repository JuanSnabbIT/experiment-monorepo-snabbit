        ...
        onSubmit: async (values) => {
            try {
                ...
                if (values.opcion === "imagen") {
                    ...
                    data = {compra: detalleCompra?.id, imagenes: values.imagenes, observaciones: values.observaciones, opcion: values.opcion}
                }
                const response = await ApiService.fetchData({url: url, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify(data)})
                if (response.data) {
                ...
                toast.error(mensajesError || "Error al subir las imagenes", {toastId: "Error al subir las imagenes"})
            }
        }
    })
                ...
                <ModalBody>
                    <div className="flex flex-col gap-4">
                                ...
                                <SelectReact
                                    ...
                                    onChange={(e) => {
                                        if (e) {
                                            ...
                                            formik.setFieldValue("opcion", (e as TSelectOption).value)
                                        }
                                    }}
                                    onBlur={formik.handleBlur}
                                    value={OPCIONES_IMAGEN.find(ar => ar.value === formik.values.opcion)}
                        ...
                        {formik.values.opcion === "imagen" && (
                            <div>
                                ...
                                >
                                    <SelectReact
                                        name="item"
                                        options={listaItemsCompra.map(item => ({value: item.item.toString(), label: item.nombre_item}))}
                                        onChange={(e) => {
                                            if (e) {
                                        ...
                                        onBlur={formik.handleBlur}
                                        value={{value: formik.values.item, label: listaItemsCompra.find(item => item.item.toString() === formik.values.item)?.nombre_item || ""}}
                                    />
                                </Validation>
                            </div>
                        )}
                        ...
                        {!permissionChecked && formik.values.opcion != "" && (
                            <Button variant="solid" onClick={() => {checkCameraPermission()}}>Abrir Camara</Button>
                        )}
                        {!hasCameraPermission && (
                            <div>No hay camara</div>
                        )}
