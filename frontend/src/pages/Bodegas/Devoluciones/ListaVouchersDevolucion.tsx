...
const ListaVouchersDevolucion = () => {
    ...
    const ordenTrabajoId = useMemo(
        () => (ordenTrabajoParam ? parseInt(ordenTrabajoParam, 10) : undefined),
        [ordenTrabajoParam],
    );

    const {
        listaVouchers = [],
        vouchersPaginacion = { count: 0, next: null, previous: null },
        loading,
        error,
    ...
    const handleCrearVoucherAhora = async () => {
        ...
        try {
            ...
            const anyErr = err as {
                response?: { status?: number; data?: { detail?: string; voucher_id?: number } };
                message?: string;
            };
            const status = anyErr?.response?.status;
            const msg =
                anyErr?.response?.data?.detail || anyErr?.message || 'No se pudo crear el voucher';
        ...
        columnHelper.accessor('total_items_devueltos', {
            ...
            cell: (info: VoucherCellContext<number>) => (
                <Badge color='blue' variant='solid'>
                    ...
                    {(info.getValue() as number) === 1 ? 'item' : 'items'}
                </Badge>
            ),
            size: 150,
        }),
                ...
                <div className='flex gap-2'>
                    ...
                    <Tooltip text='Descargar PDF'>
                        <Button
                            ...
                            color='red'
                            onClick={() =>
                                handleDescargarPDF(info.row.original.id, info.row.original.numero)
                            }
                        />
                    </Tooltip>
                </div>
                        ...
                        {ordenTrabajoId && (
                            <Tooltip text='Agrupa todas las devoluciones registradas en esta OT'>
                                <Button
                                    ...
                                    color='blue'
                                    icon='HeroPlusCircle'
                                    onClick={handleCrearVoucherAhora}>
                                    Crear voucher ahora
                                </Button>
                            </Tooltip>
                        )}
                        ...
                        <CardBody className='z-0'>
                            {loading ? (
                                ...
                                <div className='py-10 text-center text-zinc-600'>
                                    <div className='mb-3'>
                                        No hay vouchers de devolucion para los filtros actuales.
                                    </div>
                                    {ordenTrabajoId && (
                                        <Button
                                            color='blue'
                                ...
                                <div className='overflow-auto'>
                                    <Table className='min-w-[900px] table-fixed'>
                                        <THead>
                                            {table
                                                .getHeaderGroups()
                                                .map((headerGroup: VoucherHeaderGroup) => (
