...
const DetalleVoucherDevolucion = () => {
			...
			<PageWrapper isProtectedRoute name="Detalle Voucher Devolución" title="Cargando...">
				<div className="flex items-center justify-center py-12">
					<div className="text-center">
						<Icon icon="HeroEllipsisHorizontal" className="mx-auto h-8 w-8 animate-spin text-blue-500" />
	...
	return (
		<PageWrapper isProtectedRoute name="Detalle Voucher Devolución" title={`Voucher ${detalleVoucher.numero}`}>
			<Subheader>
				<SubheaderLeft>
			...
			<Container>
					...
					<CardBody>
						...
						<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
							<div className="rounded-lg border border-gray-200 p-3">
								...
								<p className="mt-1 font-semibold text-gray-900">{detalleVoucher.numero}</p>
							</div>
							<div className="rounded-lg border border-gray-200 p-3">
								<p className="text-sm text-gray-600">Orden de Trabajo</p>
								<p className="mt-1 font-semibold text-blue-600">OT #{detalleVoucher.orden_trabajo}</p>
							</div>
							<div className="rounded-lg border border-gray-200 p-3">
						...
						<div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
							<div className="rounded-lg border border-gray-200 p-3">
								...
								<p className="mt-1 flex items-center gap-2 text-gray-900">
									...
									<span className="font-semibold">
										{detalleVoucher.total_items_devueltos === 1 ? 'item' : 'items'}
									</span>
								</p>
							</div>
							{detalleVoucher.observaciones && (
								<div className="rounded-lg border border-gray-200 p-3">
									...
									<p className="mt-1 font-semibold text-gray-900">{detalleVoucher.observaciones}</p>
								</div>
							)}
						</div>
					</CardBody>
				...
				{detalleVoucher.movimientos_agrupados && detalleVoucher.movimientos_agrupados.length > 0 && (
					<Card>
						<CardBody>
							<h2 className="mb-4 text-lg font-semibold text-gray-800">Movimientos</h2>
							<div className="space-y-4">
								{detalleVoucher.movimientos_agrupados.map((grupo, idx: number) => (
									<div key={idx} className="rounded-lg border border-gray-200 p-4">
										<div className="mb-3 flex items-center justify-between">
											...
											<Badge color="blue" variant="outline">
												{grupo.cantidad} {grupo.cantidad === 1 ? 'item' : 'items'}
											</Badge>
										</div>

										{grupo.movimientos && grupo.movimientos.length > 0 && (
											<div className="space-y-2">
												{grupo.movimientos.map((mov, movIdx: number) => (
													<div
														key={movIdx}
													...
													>
														<div>
															<p className="font-semibold text-gray-900">
																{mov.item_nombre || `Item #${mov.item_id}`}
															</p>
															{mov.descripcion && (
																<p className="text-xs text-gray-600">{mov.descripcion}</p>
															)}
														</div>
														<p className="font-semibold text-gray-900">{mov.cantidad}</p>
													</div>
												))}
											</div>
										)}
				...
				{(!detalleVoucher.movimientos_agrupados || detalleVoucher.movimientos_agrupados.length === 0) && (
					<Card>
						<CardBody>
							<div className="rounded-lg bg-yellow-50 p-4">
								<p className="text-yellow-800">No hay movimientos registrados para este voucher</p>
							</div>
						</CardBody>
					</Card>
				)}
