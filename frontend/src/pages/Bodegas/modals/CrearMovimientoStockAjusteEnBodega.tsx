...
function CrearMovimientoStockAjusteEnBodega({item_stock} : {item_stock: IStockItemEnBodega}) {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            cantidad: Yup.number().required("Requerido").nonNullable("Requerido").min(-1, "Minimo 0"),
            descripcion: Yup.string().required("Requerido").nonNullable("Requerido")
        }),
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/movimientos-stock/crear-ajuste/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                    ...values,
    ...
    return (
        <>
            <Tooltip text="Hacer Ajuste Manual">
                <Button variant="solid" icon="HeroArrowPath" color="zinc" onClick={() => {setIsOpen(true)}} />
            </Tooltip>
