...
import { listaAcuerdosBaseThunk, listaFirmasConfidencialidadThunk, listaUsuariosTodoElClienteThunk, useAppDispatch, useAppSelector } from "@/store"
...
function CrearConfidencialidadContrato() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            if (isFirmado && esArchivo === 0 && !values.archivo_firma) {
                toast.error("No ha subido ninguna archivo para la firma", {toastId: "No ha subido ninguna archivo para la firma"});
                return;
            }
            ...
            const archivoFirma = isFirmado ? (esArchivo === 0 ? values.archivo_firma : sigCanvas.current?.toDataURL("image/png")) : null;
            ...
            try {
                const response = await ApiService.fetchData({url: `/api/contratos/${detalleContratoEmpresaCliente?.id}/firmas/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify(payload)});
                if (response.data) {
                ...
                toast.error(error.response?.data || "Error al crear la firma de confidencialidad", {toastId: "Error al crear la firma de confidencialidad"});
            }
        }
    })
    ...
    return (
        <>
            <Tooltip text="Crear Firma de Confidencialidad">
            ...
            <Modal isOpen={isOpen} setIsOpen={setIsOpen} isStaticBackdrop={true}>
                <ModalHeader>
                    <Badge className="text-xl">Crear Firma de Confidencialidad</Badge>
                </ModalHeader>
                <ModalBody>
                    <div className="flex flex-col gap-4">
                        <div>
                            ...
                            >
                                <SelectReact
                                    name="acuerdo_base"
                                    options={listaAcuerdosBase.map(acuerdo => ({value: acuerdo.id.toString(), label: acuerdo.titulo}))}
                                    value={{value: formik.values.acuerdo_base, label: listaAcuerdosBase.find(acuerdo => acuerdo.id.toString() === formik.values.acuerdo_base)?.titulo || ""}}
                                    onChange={(e) => {formik.setFieldValue("acuerdo_base", (e as TSelectOption).value)}}
                                />
                            </Validation>
                        </div>
                        <div>
                            <Badge>Contenido</Badge>
                            <div className="ml-4">{listaAcuerdosBase.find(acuerdo => acuerdo.id.toString() === formik.values.acuerdo_base)?.contenido || "Seleccione un Acuerdo"}</div>
                        </div>
                        <div>
                        ...
                        <div>
                            <Badge>Firmado</Badge>
                            <RadioGroup isInline>
                                <Radio label="Si" name={"firmado"} selectedValue={formik.values.firmado} value={"true"} onChange={() => {formik.setFieldValue("firmado", "true")}}></Radio>
                                <Radio label="No" name={"noFirmado"} selectedValue={formik.values.firmado} value={"false"} onChange={() => {formik.setFieldValue("firmado", "false")}}></Radio>
                            </RadioGroup>
                        </div>
                        <div>
                            <Badge>Usuario</Badge>
                            ...
                            >
                                <SelectReact
                                    ...
                                    onBlur={formik.handleBlur}
                                    onChange={(e) => {formik.setFieldValue('firma_usuario_empresa', (e as TSelectOption).value)}}
                                    value={{value: formik.values.firma_usuario_empresa, label: listaUsuariosTodoElCliente.find(user => user.id.toString() === formik.values.firma_usuario_empresa)?.nombre_usuario || ""}}
                                    />
                            </Validation>
                        </div>
                        <div>
                            <RadioGroup isInline>
                                <Radio label="Archivo" name={"esArchivo"} selectedValue={esArchivo} value={0} onChange={() => {setEsArchivo(0); clear()}}></Radio>
                                <Radio label="Firma" name={"noEsArchivo"} selectedValue={esArchivo} value={1} onChange={() => {setEsArchivo(1); clear()}}></Radio>
                            </RadioGroup>
                        </div>
                        <div>
                                        ...
                                        onChange={(e) => {
                                            if (e.target.files && e.target.files[0]) {
                                                ...
                                                reader.onload = () => {
                                                    ...
                                                    formik.setFieldValue("archivo_firma", base64String)
                                                };
...
export default CrearConfidencialidadContrato