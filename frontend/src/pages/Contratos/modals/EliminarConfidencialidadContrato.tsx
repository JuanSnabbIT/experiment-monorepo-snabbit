    ...
    return (
        <>
            <Tooltip text="Eliminar Firma de Confidencialidad">
                <Button variant="solid" color="red" icon="HeroTrash" onClick={() => {setIsOpen(true)}}></Button>
            </Tooltip>
            <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
                <ModalHeader>
                    <Badge className="text-xl">¿Esta seguro(a) que desea de eliminar esta firma?</Badge>
                </ModalHeader>
                <ModalBody>
                ...
                <ModalFooter>
                    ...
                    <ModalFooterChild>
                        ...
                        <Button variant="solid" color="red" onClick={async () => {
                            try {
                                const response = await ApiService.fetchData({url: `/api/contratos/${detalleContratoEmpresaCliente?.id}/firmas/${firma.id}/`, method: 'delete'})
                                if (response.status === 204) {
                                    dispatch(listaFirmasConfidencialidadThunk({id_contrato: detalleContratoEmpresaCliente?.id}))
                                    setIsOpen(false)
                                }
                            } catch (error: any) {
                                toast.error(error.resposne.data || "Error al eliminar la firma", {toastId: "Error al eliminar la firma"})
                            }
                        }}>Eliminar</Button>
                    </ModalFooterChild>
                </ModalFooter>
