...
function PDFContratoServicios({contrato, componentRef} : {contrato : IContratoEmpresaCliente, componentRef: RefObject<HTMLDivElement>}) {
    ...
    return (
        <div className="h-full w-full md:max-w-[800px] border-2 border-black">
            <div className="flex flex-col gap-4 p-8" ref={componentRef}>
                ...
                <div className="font-bold text-xl md:text-2xl">2.- TIPO DE PLAN Y SERVICIOS</div>
                    ...
                    {contrato.contrato_servicios.length > 0 ? (
                        contrato.contrato_servicios.map((servicio, index) => (
                            <Fragment key={index}>
                                ...
                                <div className="ml-8">Valor Fijo: ${Number(servicio.precio_unitario) * servicio.cantidad}</div>
                            </Fragment>
                        ))
                    ) : (
