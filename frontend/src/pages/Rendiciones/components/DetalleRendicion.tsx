...
const DetalleRendicion = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                toast.error(mensajesError || "Error al crear la empresa", {toastId: "Error al crear la empresa"})
            }
        }
    })
    ...
    const columns = [
        ...
        columnHelper.display({
            id: "detalle",
            cell: (info) => (
                <div>{"codigo" in info.row.original.detalle_data ? info.row.original.detalle_data.codigo : "detalle" in info.row.original.detalle_data ? info.row.original.detalle_data.detalle : "Sin detalle"}</div>
            ),
            header: "Detalle"
        }),
        columnHelper.display({
            id: "categoria",
            cell: (info) => (
                <div>{"codigo" in info.row.original.detalle_data ? "Compra" : info.row.original.detalle_data.nombre_categoria}</div>
            ),
            header: "Categoria"
        }),
        columnHelper.display({
            id: "cantidad",
            cell: (info) => (
                <div>{"codigo" in info.row.original.detalle_data ? `${info.row.original.detalle_data.items.length} Items` : info.row.original.detalle_data.cantidad}</div>
            ),
            header: "Cantidad"
        }),
        columnHelper.display({
            id: "monto_unitario",
            cell: (info) => (
                <div>{"codigo" in info.row.original.detalle_data ? "No Disponible" : `$${info.row.original.detalle_data.monto_unitario}`}</div>
            ),
        ...
        columnHelper.display({
            id: "Monto Total",
            cell: (info) => (
                <div>${"codigo" in info.row.original.detalle_data ? info.row.original.detalle_data.total_compra : info.row.original.detalle_data.monto_total}</div>
            ),
            header: "Monto Total"
        }),
        columnHelper.display({
            id: "fecha_gasto",
            cell: (info) => (
                <div>{dayjs("codigo" in info.row.original.detalle_data ? info.row.original.detalle_data.fecha_creacion : info.row.original.detalle_data.fecha_gasto).locale("es").format("DD/MM/YYYY")}</div>
            ),
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardHeaderChild>
                                        ...
                                        {editando ? (
                                            <div className="flex gap-2">
                                                <Tooltip text='Guardar Cambios'>
                                                    <Button variant='solid' icon='HeroCheck' onClick={() => {formik.handleSubmit()}}/>
                                                </Tooltip>
                                                <Tooltip text='Cancelar'>
                                                    <Button color='red' variant='solid' onClick={() => {setEditando(false)}} icon='HeroXMark'/>
                                                </Tooltip>
                                            </div>
                                        ) : (
                                            <Tooltip text="Editar">
                                ...
                                {detalleRendicion && (detalleRendicion.estado != "0" && detalleRendicion.estado != "3") && (
                                    <Tooltip text="Descargar PDF">
                                        <Button variant='solid' color='red' icon='DuoDownloadedFile' onClick={async () => {
                                            try {
                                                const response = await ApiService.fetchData<BlobPart>({url: `/api/rendiciones/${detalleRendicion.id}/descargar-pdf`, method: 'get', headers: {'Content-Type': 'application/pdf'}})
                                                ...
                                                toast.error(error.response.data || "Error al descargar el pdf", {toastId: "Error al descargar el pdf"})
                                            }
                                        }} />
                                    </Tooltip>
                                )}
                                ...
                                {editando ? (
                                    ...
                                    <>
                                        <div className="w-full">
                                            <Badge>Fecha de la Rendición</Badge>
                                            <div className="ml-4">{detalleRendicion?.fecha_rendicion ? dayjs(detalleRendicion.fecha_rendicion).format('DD/MM/YYYY') : ''}</div>
                                        </div>
                                        <div className="w-full">
                                            <Badge>Observaciones</Badge>
                                            <div className="ml-4">{detalleRendicion?.observaciones || "Sin Observaciones"}</div>
                                        </div>
                                        <div className='w-full'>
                                            <Badge>Nombre del Usuario</Badge>
                                            <div className="ml-4">{detalleRendicion?.datos_usuario.nombre_usuario}</div>
                                        </div>
                                        <div className="w-full">
                                            <Badge>Rut del Usuario</Badge>
                                            <div className="ml-4">{detalleRendicion?.datos_usuario.papeleta.rut || "Sin Rut"}</div>
                                        </div>
                                    </>
                                )}
                            </div>
                            ...
                            <CardHeaderChild>
                                <div className="flex justify-between items-center">
                                    <AnimacionDeInputModoMovil globalFilter={globalFilter} setGlobalFilter={setGlobalFilter} anchoInput={200}>
                                        {detalleRendicion && detalleRendicion.estado === "0" && <CrearItemRendicion /> }
