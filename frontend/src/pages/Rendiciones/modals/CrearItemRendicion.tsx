...
import { detalleRendicionThunk, listaCategoriasGastoThunk, listaComprasDisponiblesThunk, listaContentTypeThunk, listaDetalleGastoRendicionOTDisponiblesThunk, listaItemsRendicionThunk, useAppDispatch, useAppSelector } from "@/store"
...
function CrearItemRendicion() {
    ...
    const { listaDetalleGastoRendicionOTDisponibles } = useAppSelector((state) => state.ordenTrabajo)
    const { listaCategoriasGasto, detalleRendicion, listaComprasDisponibles } = useAppSelector((state) => state.rendicion)
    const { listaContentType } = useAppSelector((state) => state.core)
    const [isOpen, setIsOpen] = useState<boolean>(false)
    const [creandoItem, setCreandoItem] = useState<boolean>(false)
    const [optionsItem, setOptionsItem] = useState<{label: string, options: {value: string, label: string, ct: string}[]}[]>([])
    ...
    useEffect(() => {
        ...
        if (listaComprasDisponibles.length > 0) {
            ...
            options.push({label: "Compras", options: listaComprasDisponibles.map(com => ({value: com.codigo, label: `Codigo: ${com.codigo}`, ct: ct?.id.toString() || ""}))})
        }
        if (listaDetalleGastoRendicionOTDisponibles.length > 0) {
            let ct = listaContentType.find(ct => ct.model === "detallegastorendicionot")
            options.push({label: "Gastos OT", options: listaDetalleGastoRendicionOTDisponibles.map(det => ({value: det.id.toString(), label: det.detalle || "", ct: ct?.id.toString() || ""}))})
        }
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                let data = {}
                if (creandoItem) {
                    data = {detalle: values.detalle, categoria: values.categoria, cantidad: values.cantidad, monto_unitario: values.monto_unitario, fecha_gasto: values.fecha_gasto}
                } else {
                    data = {detalle_id: values.detalle_id, content_type: values.content_type}
                }
                const response = await ApiService.fetchData({url: `/api/rendiciones/${detalleRendicion?.id}/items-rendicion/crear-item/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify(data)})
                if (response.data) {
                ...
                toast.error(mensajesError || "Error al crear el item de la rendición", {toastId: "Error al crear el item de la rendición"})
            }
        }
    })
    ...
    const todasLasOpciones = useMemo(
        () => optionsItem.flatMap(g => g.options),
        [optionsItem]
    );
                        ...
                        <div>
                            ...
                            >
                                <SelectReact
                                    ...
                                    options={optionsItem}
                                    value={creandoItem ? {value: formik.values.detalle_id, label: formik.values.detalle} : opcionSeleccionada}
                                    formatCreateLabel={(e) => (`Crear detalle "${e}"`)}
                                    onChange={(e: any) => {
                                        if (e) {
                                            setCreandoItem(false)
                                            formik.setFieldValue("content_type", e.ct)
                                            formik.setFieldValue("detalle_id", (e as TSelectOption).value)
