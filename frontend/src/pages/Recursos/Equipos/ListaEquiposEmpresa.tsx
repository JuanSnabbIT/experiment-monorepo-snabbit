        ...
        columnHelper.accessor("nombre_equipo", {
            cell: (info) => (
                <p>
                    ...
                    <span style={{ color: (info.row.original.nombre_usuario_asignado || "Sin usuario") === "Sin usuario" ? "red" : "green" }}>
            ...
            <Subheader>
                ...
                <SubheaderRight>
                    {listaMisClientes.length > 5 ? (
                        <SelectReact
                            options={listaMisClientes.map(cliente => ({value: String(cliente.info_cliente.id), label: cliente.info_cliente.nombre}))}
                            isClearable
                            value={clienteSeleccionado}
                            onChange={(newValue) => {
                                const selectedOption = newValue as SingleValue<{ value: string; label: string }> | null;
                        ...
                        <>
                            <div className="flex flex-row gap-4 overflow-auto">
                                {listaMisClientes.map((cliente, index) => (
                                    <Button
                                        key={index}
                                        {...(activeComponent === cliente.info_cliente.nombre ? { ...activeProps } : { ...defaultProps })}
                                        onClick={async () => {
                                            setActiveComponent(cliente.info_cliente.nombre);
                                            dispatch(listaEquiposPorClienteThunk({cliente_id: cliente.info_cliente.id}))
                                    }}>{cliente.info_cliente.nombre}</Button>
                                ))}
                            </div>
                        </>
                    )}
                                            ...
                                            <Tr>
                                                <Td colSpan={columns.length} className="text-center">
                                                    <div className="flex flex-col items-center justify-center">
                                                        <Icon icon="HeroFaceFrown" className="mb-2 text-red-500" size="text-6xl"/>
                                                        <Badge className="text-xl">El cliente seleccionado no tiene equipos.</Badge>
                                                    </div>
                                                </Td>
                                            </Tr>
                                        )
