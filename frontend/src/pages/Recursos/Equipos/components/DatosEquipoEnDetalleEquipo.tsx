...
import { CONDICIONES_EQUIPO, MARCA_EQUIPO, SISTEMA_OPERATIVO, TAMANIO_RAM, TIPO_EQUIPO, GENERACION_PROCESADOR, TIPO_PROCESADOR, MARCA_TARJETA_GRAFICA, TIPO_TARJETA_GRAFICA } from '@/constants/recursos.constant';
...
import { detalleEquipoEmpresaThunk, listaUsuariosTodaLaEmpresaThunk, useAppDispatch, useAppSelector } from '@/store';
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const data = {
                    ...values,
                    fecha_compra: values.fecha_compra ? dayjs(values.fecha_compra).format("YYYY-MM-DD") : null,
                    fecha_caducidad_garantia: values.fecha_caducidad_garantia ? dayjs(values.fecha_caducidad_garantia).format("YYYY-MM-DD") : null
                }
                const response = await ApiService.fetchData({url: `/api/equipos/${detalleEquipoEmpresa?.id}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify(data)})
                if (response.data) {
                        ...
                        <CardHeader>
                            ...
                            {isEditing ? (
                                <>
                                    ...
                                    <Button variant="solid" color="red" onClick={() => {setIsEditing(false)}}>Cancelar</Button>
                                </>
                            ) : (
                                <Button variant="solid" onClick={() => {setIsEditing(true)}}>Editar</Button>
                            )}
                            </CardHeaderChild>
                        </CardHeader>
