...
const ProfilePage = () => {
	...
	const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
			...
			rut: Yup.string()
				...
				.matches(/^\d+(-[\dkK])$/, 'El formato del RUT no es válido'),
			celular: Yup.string()
		...
		onSubmit: async (values) => {
            try {
				const new_tema = values.theme === "light" ? "1" : values.theme === "dark" ? "2" : values.theme === "system" ? "3" : ""
				if (values.theme != personalizacionUsuario?.tema) {
					const responseTema = await ApiService.fetchData({url: `/api/personalizacion-usuarios/${personalizacionUsuario?.id}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({tema: new_tema})})
				...
				toast.error(error.response.data || "Error al actualizar el perfil", {toastId: "Error al actualizar el perfil"})
            }
			formik.setSubmitting(false)
        },
	});

	const formikContacto = useFormik({
		enableReinitialize: true,
		initialValues: {
		...
		onSubmit: async (values) => {
			try {
				const response  = await ApiService.fetchData({url: `/auth/users/${userData?.pk}/`, method: 'patch', headers: {'Content-Type': 'application/json'}, data: JSON.stringify(values)})
				if (response.data) {
					toast.success("Se actualizo el contacto", {autoClose: 1000})
					dispatch(userMeThunk({access}))
				}
			} catch (error: any) {
				toast.error(error.response.data || "Error al editar el contacto del perfil", {toastId: "Error al editar el contacto del perfil"})
			}
		}
	})
	...
	useEffect(() => {
		if (formikContacto.values.region != 0) {
			const filteredProvincias = listaProvincias.filter(
				(provincia) => provincia.provincia_region === formikContacto.values.region
			);
			...
			formikContacto.setFieldValue('comuna', 0)
			formikContacto.setFieldValue('provincia', 0)
		}
	}, [formikContacto.values.region]);

	useEffect(() => {
		if (formikContacto.values.provincia != 0) {
			const filteredComunas = listaComunas.filter(
				(comuna) => comuna.comuna_provincia === formikContacto.values.provincia
			);
	...
	}, [formikContacto.values.provincia]);
			...
			<Subheader>
				...
				<SubheaderRight>
					...
					{/* <Button
						...
						onClick={() => {formik.handleSubmit(); formikContacto.handleSubmit()}}>
						{saveBtnText}
					</Button> */}
				</SubheaderRight>
			</Subheader>
								...
								{activeTab === TAB.EDIT && (
									<>
										<div className="flex justify-between">
											...
											<Button variant='solid' onClick={() => {formik.setSubmitting(true); formik.handleSubmit()}} isDisable={formik.isSubmitting}>Guardar</Button>
										</div>
								...
								{activeTab === TAB.CONTACTO && (
									<>
										<div className="flex justify-between">
											...
											<Button variant='solid' onClick={() => {formikContacto.setSubmitting(true); formikContacto.handleSubmit()}} isDisable={formik.isSubmitting}>Guardar</Button>
										</div>
										<div className='grid grid-cols-12 gap-4'>
                                            <div className='col-span-12'>
												<Label htmlFor='direccion'>Direccion</Label>
                                                <Validation
                                                    isValid={formikContacto.isValid}
                                                    isTouched={formikContacto.touched.direccion}
                                                    invalidFeedback={formikContacto.errors.direccion}
                                                >
                                                    <Input
                                                        id='direccion'
                                                        name='direccion'
                                                        onChange={formikContacto.handleChange}
                                                        value={formikContacto.values.direccion || ""}
                                                    />
                                                </Validation>
											</div>
										</div>
										<div className='grid grid-cols-12 gap-4'>
                                            <div className='col-span-12'>
                                                ...
                                                <Validation
                                                    isValid={formikContacto.isValid}
                                                    isTouched={formikContacto.touched.region}
                                                    invalidFeedback={formikContacto.errors.region}
                                                >
                                                    <SelectReact 
														id="region"
														name="region"
														...
														onBlur={formikContacto.handleBlur}
														onChange={(e) => {formikContacto.setFieldValue('region', parseInt((e as TSelectOption)?.value))}}
														value={{value: formikContacto.values.region?.toString() || "0", label: optionsRegion.find(region => region.value == formikContacto.values.region?.toString())?.label || ""}}
													/>
                                                </Validation>
											</div>
										</div>
										<div className='grid grid-cols-12 gap-4'>
                                            <div className='col-span-12'>
                                                ...
                                                <Validation
                                                    isValid={formikContacto.isValid}
                                                    isTouched={formikContacto.touched.provincia}
                                                    invalidFeedback={formikContacto.errors.provincia}
                                                >
                                                    <SelectReact 
														id="provincia"
														name="provincia"
														...
														onBlur={formikContacto.handleBlur}
														onChange={(e) => {formikContacto.setFieldValue('provincia', parseInt((e as TSelectOption)?.value))}}
														value={{value: formikContacto.values.provincia?.toString() || "0", label: optionsProvincia.find(provincia => provincia.value == formikContacto.values.provincia?.toString())?.label || ""}}
													/>
                                                </Validation>
											</div>
										</div>
										<div className='grid grid-cols-12 gap-4'>
                                            <div className='col-span-12'>
                                                ...
                                                <Validation
                                                    isValid={formikContacto.isValid}
                                                    isTouched={formikContacto.touched.comuna}
                                                    invalidFeedback={formikContacto.errors.comuna}
                                                >
                                                    <SelectReact 
														id="comuna"
														name="comuna"
														...
														onBlur={formikContacto.handleBlur}
														onChange={(e) => {formikContacto.setFieldValue('comuna', parseInt((e as TSelectOption)?.value))}}
														value={{value: formikContacto.values.comuna?.toString() || "0", label: optionsComuna.find(comuna => comuna.value == formikContacto.values.comuna?.toString())?.label || ""}}
													/>
                                                </Validation>
											</div>
										</div>
