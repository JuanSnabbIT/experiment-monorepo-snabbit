...
import { listaDiasCalendarioThunk, listaSolicitudesVacacionesUsuarioThunk, useAppDispatch, useAppSelector } from "@/store"
...
function PedirVacaciones() {
    ...
    useEffect(() => {
        ...
        if (listaSolicitudesVacacionesUsuario.filter(sol => sol.estado === '2').length > 0 && usuarioSeleccionado) {
                                    ...
                                    <Table className='table-fixed'>
                                        ...
                                        <TBody>
                                            {table.getRowModel().rows.map((row) => (
                                                <Tr className={`hover:cursor-pointer ${formik.values.usuario_empresa === row.original.id ? "bg-blue-500 text-white" : ""}`}
                                                    key={row.id}
                                                    onClick={() => {
                                                        if (formik.values.usuario_empresa === row.original.id) {
                                                            formik.setFieldValue('usuario_empresa', 0);
                                                            setUsuarioSeleccionado(undefined)
                                                        } else {
                                                            formik.setFieldValue('usuario_empresa', row.original.id);
