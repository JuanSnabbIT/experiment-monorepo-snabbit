...
function ListaDiasCalendarioV2() {
	...
	const { viewMode, changeViewMode, next, prev, today, title: currentDate, handleDatesSet } = useCalendarView(ref);
	...
	useEffect(() => {
		...
		const eventosCombinados = [...eventosDiasCalendario, ...eventosSolicitudesVacaciones, ...eventosOrdenDeTrabajo];
	...
	const handleEventClick = (clickInfo: EventClickArg) => {
		...
		if (clickInfo.event.extendedProps.tipo_evento === "calendario" && clickInfo.event.start) {
			...
			setIsOpenModalDia(true)
		} else if (clickInfo.event.extendedProps.tipo_evento === "orden_trabajo" && clickInfo.event.start) {
		...
		return (
			<>
				...
				{eventContent.event.extendedProps.tipo_evento === "orden_trabajo" ? (
					<div className="p-1">Orden de Trabajo N°{eventContent.event.extendedProps.id} - {eventContent.event.title}</div>
				) : (
	...
	return (
		<PageWrapper isProtectedRoute={true} title="Calendario" name="Calendario">
			...
			>
				<ModalHeader>
					<Badge className="text-xl">Dia {dayjs(fechaSeleccionada).format('DD-MM-YYYY')}</Badge>
				</ModalHeader>
				<ModalBody>
							...
							<div className="grid grid-cols-1 gap-4">
								...
								<div className="flex justify-between">
									...
									<div className="gap-4 flex">
										<Button onClick={() => { setIsOpenModalDia(false); formik.resetForm() }}>Cancelar</Button>
							...
							<div className="grid grid-cols-1 gap-4">
								<div className="flex flex-col">
									...
									<div className="flex justify-end mt-4">
										<Button onClick={() => { setIsOpenModalDia(false); formikCrear.resetForm()}}>Cancelar</Button>
