...
function PedirVacacionesUsuario() {
    ...
    const { listaDiasCalendario, listaSolicitudesVacacionesUsuario } = useAppSelector((state) => state.calendario)
    ...
    useEffect(() => {
        ...
        if (listaSolicitudesVacacionesUsuario.filter(sol => sol.estado === '2').length > 0 && usuarioEmpresaLogeado) {
            ranges.push(...listaSolicitudesVacacionesUsuario.filter(sol => sol.estado === '2').map((sol, index) => ({
        ...
        if (listaDiasCalendario.filter(dias => dias.es_feriado).length > 0) {
            ranges.push(...listaDiasCalendario.filter(dias => dias.es_feriado).map((dias, index) => ({
    ...
    const validationSchema = Yup.object().shape({
        ...
        if (extraordinaria === "false") {
            ...
            if ( fecha_inicio && fecha_fin) {
                ...
                const diasHabiles = calcularDiasHabilesConCalendario(startDate, endDate, listaDiasCalendario);

                if (usuarioEmpresaLogeado && usuarioEmpresaLogeado.papeleta.dias_disponibles < diasHabiles) {
            ...
            <Container className="w-full h-full">
                <div className="grid grid-cols-12 w-full gap-4">
                    <div className="col-span-full">
                        <Card>
                            <CardBody className="grid grid-cols-2 lg:grid-cols-4 justify-center items-center">
                                <div className="w-full">
                                    <Badge>Dias Acumulados</Badge>
                                    <div id="dias_acumulados" className="ml-4">{usuarioEmpresaLogeado ? `${usuarioEmpresaLogeado.papeleta.dias_acumulados} dias` : "Sin Usuario"}</div>
                                </div>
                                <div className="w-full">
                                    <Badge>Dias Disponibles</Badge>
                                    <div id="dias_disponibles" className="ml-4">{usuarioEmpresaLogeado ? `${usuarioEmpresaLogeado.papeleta.dias_disponibles} dias` : "Sin Usuario"}</div>
                                </div>
                                <div className="w-full">
                                    <Badge>Dias Tomados</Badge>
                                    <div id="dias_tomados" className="ml-4">{usuarioEmpresaLogeado ? `${usuarioEmpresaLogeado.papeleta.dias_tomados} dias` : "Sin Usuario"}</div>
                                </div>
                                <div className="w-full">
                                    <Badge>Dias Habiles Seleccionados</Badge>
                                    <div className="ml-4">{calcularDiasHabilesConCalendario(new Date(formik.values.fecha_inicio), new Date(formik.values.fecha_fin), listaDiasCalendario)}</div>
                                </div>
                            </CardBody>
                        </Card>
                    </div>
