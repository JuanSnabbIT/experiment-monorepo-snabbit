...
function ListaUsuarioEquipoOT({
	...
	const [usuarioFirmaSeleccionado, setUsuarioFirmaSeleccionado] = useState<IUsuarioAsignadoSoporte | null>(
		null,
	);
	...
	const usuariosAsignadosSet = useMemo(() => {
		...
		(listaUsuariosAsignadosSoporte || []).forEach((u) => {
			...
			if (u.usuario_equipo) {
				const usuarioEmpresaId = usuarioEquipoToUsuarioEmpresa.get(u.usuario_equipo.toString());
		...
		return set;
	}, [listaUsuariosAsignadosSoporte, usuarioEquipoToUsuarioEmpresa, usuariosAsignadosOtPendientes]);
	...
	const equiposInactivosEmpresa = useMemo(() => {
		...
		(listaUsuariosDelEquipoPorCliente || [])
			...
			.forEach((u) => {
				...
				if (!id || (equiposActivosSet.has(id) && !equiposLiberables.has(id)) || map.has(id)) return;
				const label = buildEquipoLabel(u.datos_equipo);
				map.set(id, { value: id, label });
			});

		return Array.from(map.values());
	}, [listaEquiposPorCliente, listaUsuariosDelEquipoPorCliente, equiposActivosSet, equiposLiberables]);
	...
	const guardarDisabled =
		guardando || (isPendiente ? seleccionPendiente.length === 0 : isEnProceso ? !hayCambiosEnProceso : true);
	...
	const abrirModalFirmaUsuario = (usuario: IUsuarioAsignadoSoporte, cache: CacheAsignacion | null) => {
	...
	const actualizarCacheSeleccion = (usuario: IUsuarioAsignadoSoporte, option: TSelectOption | null) => {
	...
	const guardarCambiosEquipos = async () => {
		...
		const actualizar = async (idUsuarioAsignado: number, data: Partial<IUsuarioAsignadoSoporte>) => {
			try {
				await dispatch(
					actualizarUsuarioAsignadoSoporteThunk({
						id_orden: ordenId,
	...
	const renderEquipoPendienteCell = (fila: FilaPendiente) => {
		...
		return <span className='text-gray-400 italic'>Sin equipo</span>;
	};

	const renderEquipoAsignadoCell = (usuario: IUsuarioAsignadoSoporte) => {
		if (!isEnProceso) {
			return usuario.numero_serie_equipo || <span className='text-gray-400 italic'>Sin equipo</span>;
		}
		if (usuario.resuelto) {
			return usuario.numero_serie_equipo || <span className='text-gray-400 italic'>Sin equipo</span>;
		}
		const firmaPendiente = Boolean(
			(usuario.cache_asignacion as Record<string, unknown> | null | undefined)?.firma_pendiente,
		);
		...
		const equiposDisponibles = equiposInactivosEmpresa
			...
			.filter((opt) => !(seleccionActual === 'none' && equipoActualId && opt.value === equipoActualId))
			.map((opt) => ({
				value: `eq:${opt.value}`,
				label: opt.label,
			}));
		...
		const value = findOption(seleccionActual || null) || (seleccionActual && fallbackLabel ? {
					...
					<div className='flex flex-col gap-4'>
						...
						<div className='rounded-lg border'>
							...
							<Table className='table-fixed w-full'>
								<THead>
									<Tr>
										<Th className='text-left w-[30%]'>Usuario</Th>
										<Th className='text-left w-[45%]'>Equipo</Th>
										<Th className='text-left w-[15%]'>Asignacion</Th>
								...
								<TBody>
									...
									{isPendiente && seleccionPendiente.map((u) => (
