            ...
            <ModalBody>
                <div className='grid grid-cols-2 gap-4'>
                    <div className='w-full'>
                        <Badge>Estado</Badge>
                        <div className='ml-4'>
                            {ESTADOS_DETALLE_TRABAJO.find(
                                (estado) => estado.value === detalleDelDetalleTrabajo?.estado,
                            )?.label || 'Sin estado'}
                        </div>
                    </div>
                    <div className='w-full'>
                        <Badge>
                            ...
                            {detalleDelDetalleTrabajo &&
                                detalleDelDetalleTrabajo.tecnico_asignado != null && (
                        ...
                        <div className='ml-4'>
                            {detalleDelDetalleTrabajo?.tecnico_asignado
                                ? detalleDelDetalleTrabajo.nombre_tecnico
                                : 'Sin Técnico'}
                        </div>
                    </div>
                    <div className='w-full'>
                    ...
                    <div className='w-full'>
                        ...
                        <div className='ml-4'>
                            {detalleDelDetalleTrabajo?.fecha_creacion
                                ? dayjs(detalleDelDetalleTrabajo.fecha_creacion)
                                      .locale('es')
                                      .format('DD/MM/YYYY')
                                : 'Sin fecha'}
                        </div>
                                    ...
                                    onClick={() => {
                                        if (
                                            listaContentType.find(
                                                (ct) =>
                                                    ct.id === detalleDelDetalleTrabajo.content_type,
                                            )?.model === 'compra'
                                        ) {
                                            navigate(
                                                `/compras/detalle-compra/${detalleDelDetalleTrabajo.trabajo_id}`,
                                            );
                                        } else if (
                                            listaContentType.find(
                                                (ct) =>
                                            ...
                                            navigate(
                                                `/orden-trabajo/detalle-visita-soporte/${detalleDelDetalleTrabajo.trabajo_id}`,
                                            );
                                        } else if (
                                            listaContentType.find(
                                                (ct) =>
                                            ...
                                            navigate(
                                                `/cotizacion/detalle-cotizacion/${detalleDelDetalleTrabajo.codigo_cotizacion}`,
                                            );
                                        } else {
                                            toast.error('No se reconoce el tipo de trabajo', {
                                                toastId: 'No se reconoce el tipo de trabajo',
                                            });
                                        }
                                    }}></Button>
                            )}
                        ...
                        {detalleDelDetalleTrabajo && detalleDelDetalleTrabajo.insumo !== null && (
                            <Tooltip text='Insumo'>
                                <Button
                                    ...
                                    onClick={() => {
                                        navigate(
                                            `/bodega/detalle-guia-salida-bodega/${detalleDelDetalleTrabajo.insumo}`,
                                        );
                                    }}></Button>
                            </Tooltip>
                        )}
