...
function CrearUsuarioAsignadoOT({
	...
	const { listaUsuariosDelEquipoPorCliente, listaEquiposPorCliente, listaEquiposDeMisClientes } = useAppSelector(
	...
	useEffect(() => {
		...
		if (modoCrearUsuarioEquipo) {
			...
			if (detalleOrdenTrabajo?.empresa) {
				dispatch(listaEquiposDeMisClientesThunk({ id_empresa: detalleOrdenTrabajo.empresa }));
			}
		} else if (!isSoporteMode) {
			dispatch(listaUsuariosTodoElClienteThunk({ id_empresa: clienteId }));
		}
	}, [clienteId, isOpen, isSoporteMode, dispatch, modoCrearUsuarioEquipo, detalleOrdenTrabajo?.empresa]);
	...
	const equiposLibresOptions = useMemo(() => {
		...
		const equiposBase = (listaEquiposPorCliente && listaEquiposPorCliente.length > 0
		...
		const todosEquipos = [...equiposBase, ...equiposDerivados, ...equiposExtra, ...equiposMisClientes];
	...
	const crearUsuarioEquipo = async () => {
		...
		try {
			...
			if (resp.data?.id) {
				...
				const numeroSerieParsed = (equipoNuevo.label || '').split(' - ')[0] || equipoNuevo.label || '';
				onUsuarioEquipoCreado?.({
			...
			<Modal isOpen={isOpen} setIsOpen={setIsOpen} isStaticBackdrop={true} size='xl'>
						...
						<div className='flex flex-col gap-4'>
							<div>
								...
								<SelectReact
									...
									onChange={(val) => handleSelectChange(val as TSelectOption | null)}
							...
							{!isSoporteMode && mostrarCampoCorreo && nombreExterno && (
								<div className='rounded-lg border bg-blue-50 p-4'>
									...
									<div className='flex gap-2'>
										...
										<Button variant='solid' color='blue' onClick={agregarUsuarioExterno}>
											Agregar
										</Button>
									</div>
								</div>
										...
										<Table>
											<THead>
												<Tr>
													...
													{!isSoporteMode && <Th className='text-left'>Correo</Th>}
													...
													<Tr key={`${usuario.nombre}-${index}`}>
														...
														<Td>
															{usuario.tipo === 'empresa' ? (
																...
																<Badge color='sky'>
																	{(usuario as UsuarioTemporalSoporte).equipo_label ||
																		'Equipo'}
																</Badge>
															)}
														</Td>
														{!isSoporteMode && (
															<Td>
																{usuario.tipo !== 'soporte' && usuario.correo ? (
																	usuario.correo
																) : (
				...
				<ModalFooter>
					...
					<ModalFooterChild>
						...
						<Button
							variant='solid'
							onClick={modoCrearUsuarioEquipo && isSoporteMode ? crearUsuarioEquipo : guardarTodos}
							isDisable={
								guardando ||
								(modoCrearUsuarioEquipo && isSoporteMode
