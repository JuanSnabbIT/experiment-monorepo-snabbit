...
function ModalConfirmarEscaneoItemCompraDT({
    ...
    useEffect(() => {
        if (isOpen && item) {
            ...
            formik.setValues({
                ...
                cantidad: 0,
                precio: 0,
                imagenes: item.imagenes.length > 0 ? item.imagenes.map(imagen => imagen.imagen) : [],
    ...
    const formik = useFormik<FormikInterface>({
        ...
        onSubmit: async (values) => {
            if (values.creando) {
                try {
                    const response = await ApiService.fetchData({url: `/api/compras/${detalleCompra?.id}/items-compras/crear-item-empresa/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                ...
                try {
                    const response = await ApiService.fetchData({url: `/api/compras/${detalleCompra?.id}/items-compras/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
            ...
            <ModalBody>
                <div className="flex flex-col gap-4">
                    <div className="flex flex-row gap-4 overflow-auto">
                        <Button
                            {...(activeComponent === "Item de la Empresa"  ? {size: 'sm', rounded: 'rounded-full', className: 'border', isActive: true,color: 'blue', colorIntensity: '500', variant: 'solid',} : {size: 'sm', color: 'zinc', rounded: 'rounded-full', className: 'border'})}
                            onClick={() => {setActiveComponent("Item de la Empresa"); formik.setFieldValue("creando", false)}}>
                            Item de la Empresa
                        </Button>
                        <Button
                            {...(activeComponent === "Crear"  ? {size: 'sm', rounded: 'rounded-full', className: 'border', isActive: true,color: 'blue', colorIntensity: '500', variant: 'solid',} : {size: 'sm', color: 'zinc', rounded: 'rounded-full', className: 'border'})}
                            onClick={() => {setActiveComponent("Crear"); formik.setFieldValue("creando", true)}} isDisable={!!item}>
                            Crear
                        </Button>
                    </div>
