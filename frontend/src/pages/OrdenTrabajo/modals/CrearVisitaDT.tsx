...
import { listaContentTypeThunk, listaDetalleTrabajoOTThunk, useAppDispatch, useAppSelector } from "@/store"
...
function CrearVisitaDT({id_detalle} : {id_detalle: string | number | undefined}) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData<IVisitaSoporte, string>({url: `/api/visitas-soporte/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                    descripcion_servicio: values.descripcion_servicio,
                    empresa: detalleOrdenTrabajo?.empresa,
                    cliente: detalleOrdenTrabajo?.cliente
                })})
                if (response.data) {
                    const responseTrabajo = await ApiService.fetchData({url: `/api/ordenes-trabajo/${detalleOrdenTrabajo?.id}/detalles-trabajo/${id_detalle}/asociar-trabajo/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                        content_type: listaContentType.find(ct => ct.model === "visitasoporte")?.id,
                ...
                toast.error(mensajesError || "Error al crear la visita", {toastId: "Error al crear la visita"})
            }
        }
    })
    ...
    return (
        <>
            <Tooltip text="Crear Visita">
                <Button variant="solid" icon="HeroPlus" color="amber" onClick={() => {setIsOpen(true)}}></Button>
            </Tooltip>
