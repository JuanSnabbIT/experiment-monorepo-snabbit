...
function CrearEntregaEquipoEnOT() {
    ...
    const { detalleDelDetalleTrabajo, detalleOrdenTrabajo } = useAppSelector((state) => state.ordenTrabajo)
    ...
    useEffect(() => {
        if (isOpen && detalleDelDetalleTrabajo && detalleOrdenTrabajo) {
            ...
            dispatch(listaDeEquiposParaEntregarThunk({id_guia_salida: detalleDelDetalleTrabajo.insumo}))
        }
