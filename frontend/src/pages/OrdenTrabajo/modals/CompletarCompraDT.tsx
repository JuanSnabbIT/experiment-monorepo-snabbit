...
import { detalleDelDetalleTrabajoThunk, detalleOrdenTrabajoThunk, listaComprasOrdenTrabajoThunk, listaItemsCompraThunk, useAppDispatch, useAppSelector, usuarioEmpresaLogeadoThunk } from "@/store"
            ...
            <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
                ...
                <ModalBody>
                    <div className="flex flex-col gap-4">
                        {listaItemsCompra.filter(item => !item.item_stock?.bodega_temporal).length > 0 ? (
                            listaItemsCompra.filter(item => !item.item_stock?.bodega_temporal).map((item, index) => (
                                <div className="ml-4 flex flex-row items-center" key={index}>
                                    <Icon icon="DuoCircle" className="mr-2" /> {item.nombre_item} no tiene una bodega asignada
                                </div>
                            ))
                        ) : (
                            <>
                ...
                <ModalFooter>
                    ...
                    <ModalFooterChild>
                        ...
                        <Button variant="solid" color="emerald" isDisable={listaItemsCompra.some(item => (item.cantidad <= 0 || item.precio <= 0 || !item.item_stock?.bodega_temporal))} onClick={async () => {
                            try {
                                const response = await ApiService.fetchData({url: `/api/ordenes-trabajo/${detalleOrdenTrabajo?.id}/detalles-trabajo/${detalleDelDetalleTrabajo?.id}/completar-compra/`, method: "post", headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
