...
function ModalConfirmarRecepcionGuia({
	...
	const handleConfirmar = async () => {
		if (!clienteSolicitanteId) {
			toast.error("No se ha identificado el cliente solicitante", { toastId: "error-solicitante" });
			return;
		}
				...
				<div className="space-y-4">
					...
					<div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
						<div className="text-sm font-medium text-blue-900 leading-relaxed space-y-2">
							<div>
								Usted <span className="font-bold">{clienteSolicitanteNombre || 'Cliente Responsable'}</span>, ¿Confirma la recepción de los items de esta guía de salida?
							</div>
						</div>
					</div>
