...
function AgregarAdjuntosFotosOT() {
    ...
    const formik = useFormik<{imagenes: string[], descripcion: string}>({
        ...
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/ordenes-trabajo/${detalleOrdenTrabajo?.id}/adjuntos/bulk/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({
                ...
                toast.error(mensajesError || "Error al agregar la imagen", {toastId: "Error al agregar la imagen"})
            }
        }
    })
