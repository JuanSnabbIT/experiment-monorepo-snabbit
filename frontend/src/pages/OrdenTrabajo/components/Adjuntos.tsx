            ...
            cell: (info) => (
                info.getValue() ? (
                    <Button variant="solid" onClick={() => {
                        if (info.row.original.archivo.split(/[?#]/)[0].split(".").pop()!.toLowerCase() === "pdf") {
                            navigate(`/orden-trabajo/${info.row.original.orden}/vista-previa-adjunto/${info.row.original.id}`)
                        } else {
                            setSelectedAdjunto(info.row.original); setIsOpen(true)
                        }
                    }}>
                        Ver archivo
                    </Button>
                        ...
                        {selectedAdjunto && (
                            <>
                                ...
                                {selectedAdjunto.archivo && (
                                    <div>
                                        <Badge>Archivo</Badge>
                                        <a className="px-4" href={selectedAdjunto.archivo} target="_blank" rel="noopener noreferrer">Descargar archivo</a>
                                    </div>
                                )}
                            </>
                        )}
                ...
                <CardHeader>
                    ...
                    <CardHeaderChild>
                        <AnimacionDeInputModoMovil globalFilter={globalFilter} setGlobalFilter={setGlobalFilter} anchoInput={160}>
                            {detalleOrdenTrabajo && (detalleOrdenTrabajo.estado === "pendiente" || detalleOrdenTrabajo.estado === "en_proceso" || detalleOrdenTrabajo.estado === "completada") && (<CrearAdjunto />)}
