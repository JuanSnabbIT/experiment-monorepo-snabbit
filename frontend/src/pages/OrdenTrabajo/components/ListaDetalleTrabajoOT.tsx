...
function ListaDetalleTrabajoOT() {
	...
	const { detalleOrdenTrabajo, listaDetalleTrabajoOT } = useAppSelector(
	...
	const columns = [
		...
		columnHelper.accessor('content_type', {
			cell: (info) => (
				<div>
					{listaContentType.find((ct) => ct.id === info.row.original.content_type)
						?.model === 'cotizacion' && 'Cotización'}
					{listaContentType.find((ct) => ct.id === info.row.original.content_type)
						?.model === 'visitasoporte' && 'Visita de Soporte'}
					{listaContentType.find((ct) => ct.id === info.row.original.content_type)
						?.model === 'compra' && 'Compra'}
				...
				return (
					<div className='flex flex-row gap-2'>
						<div>
							{info.getValue()}
							<Collapse
								isOpen={descripcionAbierto === info.row.original.id}
						...
						<div>
							<Button
								...
								variant='solid'
								icon={
									descripcionAbierto === info.row.original.id
										? 'HeroEyeSlash'
										: 'HeroEye'
								}
								color='sky'
			...
			cell: (info) => {
				...
				return (
					<div className='flex flex-wrap gap-2'>
						{info.row.original.insumo === null ||
						(info.row.original.insumo && info.row.original.estado_insumo != 'ER') ? (
							<>
			...
			cell: (info) => {
				...
				return (
					<div className='flex flex-wrap gap-2'>
						...
						{typeof info.row.original.content_type != 'number' &&
							typeof info.row.original.trabajo_id != 'number' &&
							detalleOrdenTrabajo &&
							(detalleOrdenTrabajo.estado === 'pendiente' ||
								detalleOrdenTrabajo.estado === 'en_proceso') &&
							(info.row.original.estado === 'pendiente' ||
								info.row.original.estado === 'en_proceso') &&
							typeof info.row.original.insumo != 'number' && (
								<>
						...
						{listaContentType.find((ct) => ct.id === info.row.original.content_type)
							...
							<>
								...
								{detalleOrdenTrabajo &&
									...
									info.row.original.estado != 'pendiente' &&
									info.row.original.estado != 'en_proceso' &&
									info.row.original.estado_visita === 'pendiente' && (
										<CompletarVisitaDT
											id_visita={info.row.original.trabajo_id}
										/>
									)}
							</>
						)}
						...
						{info.row.original.estado_insumo === 'ET' &&
							info.row.original.estado != 'en_proceso' &&
							info.row.original.estado != 'pendiente' && (
	...
	// Fuente de datos (solo detalles / soportes técnicos)
	let trabajos: IDetalleOrdenDeTrabajo[] = listaDetalleTrabajoOT;
