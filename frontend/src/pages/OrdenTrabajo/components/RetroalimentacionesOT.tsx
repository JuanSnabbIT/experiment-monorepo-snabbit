...
function RetroalimentacionesOT() {
    ...
    const { detalleOrdenTrabajo, listaRetroalimentacionesOT } = useAppSelector((state) => state.ordenTrabajo)
            ...
            cell: (info) => (
                <div>
                    <Tooltip text="Detalle">
                        <Button variant="solid" color="violet" icon="HeroEye" onClick={() => {navigate(`/orden-trabajo/detalle-retroalimentacion/${info.row.original.id}/`)}}></Button>
                    </Tooltip>
                </div>
            )
        })
