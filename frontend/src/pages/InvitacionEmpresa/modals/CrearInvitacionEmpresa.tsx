...
function CrearInvitacionEmpresa({sucural} : {sucural: number | string}) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await ApiService.fetchData({url: `/api/invitaciones-empresa/`, method: 'post', headers: {'Content-Type': 'application/json'}, data: JSON.stringify({...values, sucursal: sucural})})
                if (response.data) {
