...
function Home() {
    ...
    const gridClasses =
        widgetsEnabled.length > 1 ? 'md:grid-cols-2 gap-4' : 'grid-cols-1 w-full mr-2';
        ...
        <PageWrapper isProtectedRoute={true} title='Dashboard' name='Dashboard'>
            ...
            <Container className='h-full w-full'>
                ...
                <div className={`grid ${gridClasses}`}>
                    {personalizacionUsuario?.dashboard_preferences.empresa_seleccionada && (
                        <div
                            className={`order-1 w-full ${!personalizacionUsuario?.dashboard_preferences.indicadores_economicos && !personalizacionUsuario?.dashboard_preferences.actualizaciones_oc ? 'w-full' : 'md:w-full'}`}>
                            <EmpresaSeleccionada />
                        </div>
                    )}
                    {personalizacionUsuario?.dashboard_preferences.indicadores_economicos && (
