...
const UltimasActualizacionesOc = () => {
    ...
    const handleToggle = (codigo_orden: string) => {
        ...
        setTimeout(
            () => setIsOpening((prev) => ({ ...prev, [codigo_orden]: false })),
            1000,
        );
    };
                            ...
                            Object.keys(groupedEvents).map((codigo_orden) => (
                                <div key={codigo_orden} className="mb-4">
                                    <TimelineItem icon="HeroHashtag" color="blue" colorShade="500">
                                        <div className="cursor-pointer mb-2" onClick={() => handleToggle(codigo_orden)}>
                                            <div className="flex items-center justify-between p-2 rounded-md shadow-sm transition-colors duration-200">
                                                <p className="font-semibold">Cambios en: <Badge>{codigo_orden}</Badge></p>
                                                <div className='flex'>
                                                    <div className='grow'>
                                                        <Button
                                                            variant='outline'
                                                            rightIcon={isOpening[codigo_orden] ? 'HeroMinusCircle' : (openOrders[codigo_orden] ? 'HeroChevronUp' : 'HeroChevronDown')}
                                                            color={isOpening[codigo_orden] ? 'red' : undefined}
                                                            isDisable={isOpening[codigo_orden]}>
                                                        </Button>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    >
                                        <div className="pl-4">
                                            {groupedEvents[codigo_orden].map((evento, index) => (
                                                <TimelineItem key={index} icon="HeroHashtag" color="blue" colorShade="500">
                                                    <div className="p-3 rounded-md shadow-sm">
                                                        <Badge className="mb-1">{evento.tipo}</Badge>
                                                        <div className='ml-4'>
                                                        <p className="text-sm font-semibold">Usuario: {evento.usuario}</p>
                                                        <p className="text-sm text-gray-600">{new Date(evento.fecha).toLocaleString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                                                        <p className="text-sm">{evento.detalle}</p>
                                                        </div>
                                                        {evento.observacion && <p className="text-sm italic">Observación: {evento.observacion}</p>}
                                                    </div>
                                                </TimelineItem>
                                            ))}
                                        </div>
