import { IAcuerdoBase, IComuna, IContentType, IProvincia, IRegion } from "@/interface/core.interface"
...
export const listaUsuariosThunk = createAsyncThunk<IUser[], undefined, {rejectValue: string}>(
    ...
    async (_, {rejectWithValue}) => {
        try {
            const response = await ApiService.fetchData<IUser[]>({url: `/api/users/`, method: 'get'})
            return response.data
        } catch (error: any) {
...
export const listaContentTypeThunk = createAsyncThunk<IContentType[], undefined, {rejectValue: string}>(
    'core/listaContentTypeThunk',
    async (_, {rejectWithValue}) => {
        try {
            const response = await ApiService.fetchData<IContentType[]>({url: `/api/content-types/`, method: 'get'})
            return response.data
        } catch (error: any) {
            return rejectWithValue(error.response.data)
        }
...
export const listaAcuerdosBaseThunk = createAsyncThunk<IAcuerdoBase[], undefined, {rejectValue: string}>(
    'core/listaAcuerdosBaseThunk',
    async (_, {rejectWithValue}) => {
        try {
            const response = await ApiService.fetchData<IAcuerdoBase[]>({url: `/api/acuerdos-base/`, method: 'get'})
            return response.data
        } catch (error: any) {
            return rejectWithValue(error.response.data)
        }
