...
export const listaSolicitudesVacacionesUsuarioThunk = createAsyncThunk<ISolicitudVacaciones[], {filtro: URLSearchParams}, {rejectValue: string}>(
    'calendario/listaSolicitudesVacacionesUsuarioThunk',
    async ({filtro}, {rejectWithValue}) => {
        try {
            const response = await ApiService.fetchData<ISolicitudVacaciones[]>({url: `/api/solicitudes-vacaciones/por-usuario`, method: 'get', params: filtro})
