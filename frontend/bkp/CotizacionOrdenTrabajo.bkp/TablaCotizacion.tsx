...
const TablaCotizacion = ({ id_ot }: { id_ot: number | string | undefined }) => {
    ...
    const { detalleOrdenTrabajo, listaCotizacionesOT } = useAppSelector(
    ...
    useEffect(() => {
        if (detalleOrdenTrabajo) {
            dispatch(
                listaUsuariosEmpresaYClienteThunk({
                    ids_empresa: [detalleOrdenTrabajo.empresa, detalleOrdenTrabajo.cliente],
                }),
            );
        }
    }, []);
                ...
                <div className='flex gap-2'>
                    ...
                    {detalleOrdenTrabajo?.estado !== '2' && detalleOrdenTrabajo?.estado !== '4' && (
                        <ModalEliminar
                            ...
                            onDispatch={() =>
                                dispatch(
                                    listaCotizacionesOTThunk({ id_ot: detalleOrdenTrabajo?.id }),
                                )
                            }></ModalEliminar>
                    )}
                </div>
