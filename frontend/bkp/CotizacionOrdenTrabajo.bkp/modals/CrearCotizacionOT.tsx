...
import {
    listaCotizacionesOTFiltradoThunk,
...
function CrearCotizacionOT({ id_ot }: { id_ot: number | string | undefined }) {
    ...
    const [optionsCotizaciones, setOptionsCotizaciones] = useState<
    ...
    useEffect(() => {
        if (listaCotizacionesOTFiltrado) {
            setOptionsCotizaciones(
                listaCotizacionesOTFiltrado.map((cot) => ({
                    value: cot.id.toString(),
                    label: `${cot.nombre} #${cot.numero_cotizacion}`,
                })),
            );
        }
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                toast.error(error.response.data || 'Error al crear la Orden de Trabajo', {
                    toastId: 'Error al crear la OT',
                });
            }
        },
    });
                            ...
                            <Validation
                                ...
                                <SelectReact
                                    ...
                                    value={{
                                        ...
                                        label:
                                            optionsCotizaciones.find(
                                                (cot) => cot.value === formik.values.cotizacion,
                                            )?.label || '',
                                    }}
                                    onChange={(option: any) =>
                                        formik.setFieldValue('cotizacion', option?.value)
                                    }
                                />
                            </Validation>
                        </div>
