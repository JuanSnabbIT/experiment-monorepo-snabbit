...
import { listaCotizacionesOTThunk, listaCotizacionesThunk, listaUsuariosEmpresaYClienteThunk, useAppDispatch, useAppSelector } from '@/store';
...
const DetalleCotizacionOT = ({valuess}: {valuess: ICotizacionEnOT}) => {
    ...
    const { listaCotizacionesOT, detalleOrdenTrabajo } = useAppSelector((state) => state.ordenTrabajo)
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                toast.error(error.response.data || "Error al editar la cotizacion en orden de trabajo", {toastId: "Error al editar la cotizacion en orden de trabajo"});
            }
        }
    })
    ...
    return (
        <>
            <Tooltip text="Editar Cotización en Orden de Trabajo">
                <Button color='violet' variant="solid" onClick={() => {setIsOpen(true)}} icon='HeroEye' />
            </Tooltip>
                                ...
                                <div className="w-full">
                                    ...
                                    >
                                        <SelectReact 
                                            ...
                                            isClearable={true}
                                            onChange={(e: any) => {formik.setFieldValue("aceptada_por", e.value)}}
                                            options={listaUsuariosEmpresaYCliente.map((user) => ({value: user.id.toString(), label: user.nombre_usuario}))}
                                            value={{value: formik.values.aceptada_por, label: listaUsuariosEmpresaYCliente.find((user) => user.id.toString() === formik.values.aceptada_por)?.nombre_usuario || ""}}
                                        />
                                    </Validation>
                                </div>
                            </>
                    ...
                    <ModalFooterChild>
                        {isEditing ? (
                            <>
                                ...
                                <Button variant='solid' onClick={() => { formik.handleSubmit(); setIsOpen(false) }}>Actualizar</Button>
                            </>
                        ) : (
