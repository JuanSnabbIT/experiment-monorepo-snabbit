...
| **Setup: Seed Datos Base** | Pobla empresas, usuarios, items, bodegas | `python scripts/setup/seed_data.py` |
| **Setup: Seed Servicios y Planes** | Crea catálogo de servicios (7) y planes (3) | `python scripts/setup/seed_servicios.py` |
| **Setup: Seed Contratos Extras** | Crea visitas (8), licencias (12), condiciones (10) | `python scripts/setup/seed_contratos_extras.py` |
| **Setup: Seed Categorías Gastos** | Crea 24 categorías para rendiciones | `python scripts/setup/seed_categorias_gastos.py` |
| **🌟 Setup: Seed Completo (RECOMENDADO)** | ⭐ **Ejecuta todos los seeds en orden** | `python scripts/setup/seed_completo.py` |
| **🚀 Setup: Inicialización Completa del Sistema** | ⭐ **Reset + Seed Completo (todo desde cero)** | Composite task (reset → seed_completo) |
...
|------|-------------|------------|
| **🚀 Setup: Inicialización Completa del Sistema** | ⭐ Reset + Seed Completo (todo desde cero) | Reset Base Local → Seed Completo (secuencial) |
...
[
  ...
  },
  {
    "key": "ctrl+shift+i",
    "command": "workbench.action.tasks.runTask",
    "args": "Setup: Inicialización Completa del Sistema"
  }
]
```

### 🎯 Flujos Recomendados de Inicialización

#### Flujo 1: Inicialización Completa (Primer Setup o Reset Total)
**Usar cuando**: Primera vez, o necesitas borrar todo y empezar de cero.

**Task recomendada**: `Setup: Inicialización Completa del Sistema`

**Qué hace**:
1. Borra `db.sqlite3`
2. Ejecuta `makemigrations` + `migrate`
3. Ejecuta `seed_completo.py` que a su vez:
   - Crea superusuario + empresa base
   - Pobla empresas, usuarios, items, bodegas
   - Crea servicios y planes
   - Crea visitas, licencias y condiciones
   - Crea categorías de gastos

**Resultado**: Sistema completamente poblado con todos los catálogos.

#### Flujo 2: Solo Población de Datos (DB ya migrada)
**Usar cuando**: La base ya existe pero está vacía o quieres repoblar catálogos.

**Task recomendada**: `Setup: Seed Completo (RECOMENDADO)`

**Qué hace**: Ejecuta los 5 scripts de seed en orden sin tocar la DB.

#### Flujo 3: Scripts Individuales (Desarrollo/Testing)
**Usar cuando**: Solo necesitas poblar un catálogo específico.

**Tasks disponibles**:
- `Setup: Seed Datos Base` - Empresas, usuarios, items
- `Setup: Seed Servicios y Planes` - Catálogo de servicios
- `Setup: Seed Contratos Extras` - Visitas, licencias, condiciones
- `Setup: Seed Categorías Gastos` - Categorías para rendiciones

**⚠️ IMPORTANTE**: Respetar orden de dependencias (ver `MODELO_DATOS_SISTEMA.md` Sección 5).
...
- [Tareas de VS Code](./instrucciones/tasks.instructions.md)
- [Backend Instructions](./instrucciones/backend-instructions.md)
- [Frontend Instructions](./instrucciones/frontend-instructions.md)
- [Playbooks](./instrucciones/playbooks.md)
